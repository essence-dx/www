...
fn property_3_verify_at_after_expiration_fails() {
    ...
    assert_eq!(
        verifier.verify_at(&token, token.exp + 1),
        Err(AuthError::TokenExpired)
    );

    // Verify well after expiration should fail
    assert_eq!(
        verifier.verify_at(&token, token.exp + 3600),
        Err(AuthError::TokenExpired)
    );
}
...
fn property_5_token_has_unique_jti_for_revocation() {
    ...
    let token1 = generator
        .generate_access("user1")
        .expect("token generation should succeed");
    let token2 = generator
        .generate_access("user1")
        .expect("token generation should succeed");
    let token3 = generator
        .generate_access("user2")
        .expect("token generation should succeed");
...
async fn property_5_revocation_is_specific() {
    ...
    // Revoke only token1
    store
        .revoke_token(&token1.jti)
        .await
        .expect("revocation should succeed");

    // Only token1 should be revoked
    assert!(
        store.is_token_revoked(&token1.jti).await.unwrap(),
...
async fn property_5_revocation_is_idempotent() {
    ...
    store
        .revoke_token(&token.jti)
        .await
        .expect("third revocation should succeed");

    // Token should still be revoked
    assert!(
        store.is_token_revoked(&token.jti).await.unwrap(),
