...
//!
//! Run with: `cargo bench --bench htip_benchmarks -p dx-www-benchmarks`
...
fn generate_htip_payload(num_templates: usize, ops_per_template: usize) -> Vec<u8> {
    ...
    writer
        .finish_and_sign(&signing_key)
        .expect("Failed to serialize HTIP")
}
...
fn bench_htip_deserialization(c: &mut Criterion) {
    ...
    let test_cases = [("small", 5, 10), ("medium", 20, 50), ("large", 50, 100)];
    ...
    for (name, num_templates, ops_per_template) in test_cases {
        ...
        group.bench_with_input(
            ...
            |b, binary| {
                b.iter(|| {
                    let stream =
                        HtipStream::new(black_box(binary), &verifying_key).expect("deserialize");
                    black_box(stream)
                });
            },
        );
