...
fn generate_state(num_slots: usize) -> StateData {
    ...
    for i in 0..num_slots {
        state.set(
            i as u32,
            format!(
                "Value for slot {} with some additional text to make it realistic",
                i
            ),
        );
    }
...
fn bench_inflate_page(c: &mut Criterion) {
    ...
    for num_slots in slot_counts {
        ...
        let meta_tags = vec![
            ...
            (
                "keywords".to_string(),
                "benchmark, ssr, dx-www, performance".to_string(),
            ),
...
fn bench_pages_per_second(c: &mut Criterion) {
    ...
    dashboard_state.set(
        7,
        "<table><tr><th>ID</th><th>Name</th><th>Status</th></tr></table>".to_string(),
    );
    ...
    group.bench_function("dashboard_page", |b| {
        b.iter(|| {
            black_box(inflate_html(
                black_box(&dashboard_template),
                black_box(&dashboard_state),
            ))
        });
    });
    ...
    group.bench_function("blog_post_page", |b| {
        b.iter(|| {
            black_box(inflate_html(
                black_box(&blog_template),
                black_box(&blog_state),
            ))
        });
    });
