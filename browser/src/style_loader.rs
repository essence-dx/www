...
//!
//! Zero-copy CSS loader for the browser WASM runtime.
...
unsafe fn host_inject_css(_css_ptr: *const u8, _css_len: u32) {}
#[cfg(not(target_arch = "wasm32"))]
unsafe fn host_apply_class(_node_id: u32, _class_id: u16) {}
#[cfg(not(target_arch = "wasm32"))]
unsafe fn host_clear_styles() {}
...
pub unsafe extern "C" fn load_styles(ptr: *const u8, len: u32) -> u32 {
    ...
    match StyleLoader::new(data) {
        Ok(loader) => {
            ...
            unsafe {
                LOADER = Some(core::mem::transmute::<StyleLoader<'_>, StyleLoader<'_>>(
                    loader,
                ));
            }
            0
