//! DX WWW CLI - App Router TSX framework tool
...
#[derive(Parser)]
#[command(name = "dx")]
#[command(version, about = "DX WWW Framework - App Router TSX development", long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}
...
enum Commands {
    ...
    Create {
        ...
        /// Template to use (currently: default)
...
async fn cmd_create(
    ...
    if template != "default" {
        anyhow::bail!(
            "template `{}` is not implemented in the standalone wrapper yet; use `--template default`",
            template
        );
    }
...
async fn cmd_add(components: &[String], all: bool, list: bool) -> anyhow::Result<()> {
    ...
    if components.is_empty() {
        ...
        println!(
            "  {} {}",
            style("dx add --all").cyan(),
            "  # Add all components"
        );
        println!(
            "  {} {}",
            style("dx add --list").cyan(),
            " # List available components"
        );
        return Ok(());
    }
    ...
    println!(
        "{}",
        style("✓ Components added successfully!").green().bold()
    );
