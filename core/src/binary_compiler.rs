//! # Legacy Binary Compiler
//!
//! Compiles DX components into the legacy DXOB object format used by internal
//! compatibility tests and migration paths.
//!
//! ## Pipeline
//!
//! ```text
//! Source  ->  Parse (AST)  ->  Analyze  ->  Generate legacy object
//! ```
//!
//! ## Output Format
//!
    ...
    fn compile_node(&mut self, node: &TemplateNode, path: &str) -> CompilerResult<()> {
        match node {
            ...
            TemplateNode::Component(comp) => {
                ...
                let html = format!(
                    "<dx-component data-component=\"{}\"></dx-component>",
                    comp.name
                );
                ...
                // Add prop bindings
                for value in comp.props.values() {
...
pub fn deserialize_dxob(data: &[u8]) -> CompilerResult<DxObjectBinary> {
    let mut reader = DxobReader::new(data);

    if reader.read_bytes(4)? != DXOB_MAGIC {
        return Err(CompilerError::Parse("Invalid DXOB: bad magic".into()));
    }

    let version = reader.read_u8()?;
    if version != DXOB_VERSION {
        return Err(CompilerError::Parse(format!(
            "Unsupported DXOB version: {version}"
        )));
    }

    let component_type = match reader.read_u8()? {
        0x01 => ComponentType::Component,
        0x02 => ComponentType::Page,
        0x03 => ComponentType::Layout,
        value => {
            return Err(CompilerError::Parse(format!(
                "Invalid DXOB component type: {value}"
            )));
        }
    };
    ...
    for _ in 0..string_count {
        strings.push(reader.read_string_u32()?);
    }
    ...
    for _ in 0..template_count {
        ...
        let html = reader.read_string_u32()?;
        let slot_count = reader.read_u16()? as usize;
        let mut slots = Vec::with_capacity(slot_count);
        for _ in 0..slot_count {
            let id = reader.read_u16()?;
            let slot_type = match reader.read_u8()? {
                0x01 => SlotType::Text,
                0x02 => SlotType::Attribute,
                0x03 => SlotType::Element,
                0x04 => SlotType::Children,
                value => {
                    return Err(CompilerError::Parse(format!(
                        "Invalid DXOB slot type: {value}"
                    )));
                }
            };
        ...
        templates.push(CompiledTemplate { id, html, slots });
    }
    ...
    for _ in 0..binding_count {
        ...
        let expression = reader.read_string_u16()?;
        let template_id = reader.read_u16()?;
        let slot_id = reader.read_u16()?;
        let binding_type = match reader.read_u8()? {
            0x01 => BindingType::Text,
            0x02 => BindingType::Attribute,
            0x03 => BindingType::Property,
            0x04 => BindingType::Class,
            0x05 => BindingType::Style,
            0x06 => BindingType::Visibility,
            0x07 => BindingType::List,
            value => {
                return Err(CompilerError::Parse(format!(
                    "Invalid DXOB binding type: {value}"
                )));
            }
        };
    ...
    for _ in 0..event_count {
        let id = reader.read_u32()?;
        let event_type = reader.read_string_u8()?;
        let handler = reader.read_string_u16()?;
    ...
    let css_class_count = reader.read_u16()? as usize;
    let mut css_classes = Vec::with_capacity(css_class_count);
    for _ in 0..css_class_count {
        css_classes.push(reader.read_string_u16()?);
    }
...
impl<'a> DxobReader<'a> {
    ...
    fn read_bytes(&mut self, len: usize) -> CompilerResult<&'a [u8]> {
        let end = self
            .offset
            .checked_add(len)
            .ok_or_else(|| CompilerError::Parse("Invalid DXOB: offset overflow".into()))?;
        if end > self.data.len() {
            return Err(CompilerError::Parse("Invalid DXOB: truncated data".into()));
        }
    ...
    fn read_u8(&mut self) -> CompilerResult<u8> {
        Ok(self.read_bytes(1)?[0])
    }

    fn read_u16(&mut self) -> CompilerResult<u16> {
        let bytes = self.read_bytes(2)?;
        Ok(u16::from_le_bytes([bytes[0], bytes[1]]))
    }

    fn read_u32(&mut self) -> CompilerResult<u32> {
        let bytes = self.read_bytes(4)?;
        Ok(u32::from_le_bytes([bytes[0], bytes[1], bytes[2], bytes[3]]))
    }

    fn read_string_u8(&mut self) -> CompilerResult<String> {
        let len = self.read_u8()? as usize;
        self.read_string(len)
    }

    fn read_string_u16(&mut self) -> CompilerResult<String> {
        let len = self.read_u16()? as usize;
        self.read_string(len)
    }

    fn read_string_u32(&mut self) -> CompilerResult<String> {
        let len = self.read_u32()? as usize;
        self.read_string(len)
    }

    fn read_string(&mut self, len: usize) -> CompilerResult<String> {
        let bytes = self.read_bytes(len)?;
        std::str::from_utf8(bytes)
            .map(|value| value.to_string())
            .map_err(|error| CompilerError::Parse(format!("Invalid DXOB UTF-8: {error}")))
    }

    fn finish(&self) -> CompilerResult<()> {
        if self.offset == self.data.len() {
            Ok(())
        } else {
            Err(CompilerError::Parse(format!(
                "Invalid DXOB: {} trailing bytes",
...
"#;
    ...
    #[test]
    fn test_deserialize_dxob_roundtrip_core_fields() {
        let source = r#"
<script lang="rust">
let count = 0;
...
"#;
        ...
        let mut compiler = BinaryCompiler::new();
        let dxob = compiler
            .compile_source(source, "Counter", Some(BlockType::Page))
            .expect("compile");
        let decoded = deserialize_dxob(&serialize_dxob(&dxob)).expect("deserialize");

        assert_eq!(decoded.name, "Counter");
        assert_eq!(decoded.component_type, ComponentType::Page);
        assert_eq!(decoded.strings, dxob.strings);
        assert_eq!(decoded.templates.len(), dxob.templates.len());
        assert_eq!(decoded.bindings.len(), dxob.bindings.len());
        assert_eq!(decoded.events.len(), dxob.events.len());
        assert_eq!(decoded.css_classes, dxob.css_classes);
        assert!(
            decoded
                .templates
                .iter()
                .any(|template| template.html.contains("button"))
        );
        assert!(
            decoded
                .bindings
                .iter()
                .any(|binding| binding.expression == "count")
        );
    }
}
