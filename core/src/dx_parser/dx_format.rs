//! # DX Format Parser
//!
//! Parser for DX-WWW component (.tsx) and page (.html) files.
//!
//! ## File Format
//!
//! ```dx
...
fn extract_template_block(
    ...
    let expected_tag = expected_type
        .map(|t| t.tag_name())
        .unwrap_or("component|page|layout");
...
fn parse_until_terminator(
    ...
    Err(DxFormatError::UnclosedDirective(
        terminators.first().unwrap_or(&"").to_string(),
    ))
}
    ...
    *pos += 1; // Skip <
    ...
    loop {
        ...
        if attr_name.starts_with("on:") || attr_name.starts_with("on") {
            let event_name = if let Some(event_name) = attr_name.strip_prefix("on:") {
        ...
        } else if let Some(binding_name) = attr_name.strip_prefix("bind:") {
            if let AttributeValue::Dynamic(expr) = value {
                bindings.insert(binding_name.to_string(), expr);
            }
        } else if let Some(class_name) = attr_name.strip_prefix("class:") {
            if let AttributeValue::Dynamic(expr) = value {
                class_directives.insert(class_name.to_string(), expr);
            }
        } else if let Some(directive_name) = attr_name.strip_prefix("use:") {
            use_directives.push(directive_name.to_string());
        } else if let Some(transition_name) = attr_name.strip_prefix("transition:") {
    ...
    fn extract_from_node(node: &TemplateNode, classes: &mut Vec<String>) {
        match node {
            TemplateNode::Element(el) => {
                if let Some(AttributeValue::Static(class_str)) = el.attributes.get("class") {
...
fn extract_imports(scripts: &[ScriptBlock]) -> Vec<ComponentImport> {
    ...
    for script in scripts {
        ...
        // JS: import Button from './Button.tsx';
                    ...
                    for cap in regex.captures_iter(&script.content) {
                        if let Some(name) = cap.get(1) {
                            imports.push(ComponentImport {
                                ...
                                path: format!("components/{}.tsx", name.as_str()),
                            });
                        }
                    }
                }
