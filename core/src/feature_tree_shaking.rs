...
impl UsageGraph {
    ...
    pub fn shake(&mut self) -> TreeShakeStats {
        ...
        let to_remove: Vec<SymbolId> = self
            .symbols
            .keys()
...
pub const FEATURE_MODULES: &[(&str, &[&str])] = &[
    (
        "forms",
        &["dx/forms", "dx-forms", "@dx/forms", "crates/www/form"],
    ),
    (
        "query",
        &["dx/query", "dx-query", "@dx/query", "crates/www/query"],
    ),
    (
        "auth",
        &["dx/auth", "dx-auth", "@dx/auth", "crates/www/auth"],
    ),
    (
        "sync",
        &["dx/sync", "dx-sync", "@dx/sync", "crates/www/sync"],
    ),
    (
        "offline",
        &[
    ...
    (
        "a11y",
        &["dx/a11y", "dx-a11y", "@dx/a11y", "crates/www/a11y"],
    ),
    (
        "i18n",
        &["dx/i18n", "dx-i18n", "@dx/i18n", "crates/www/i18n"],
    ),
];
...
pub const FEATURE_API_PATTERNS: &[(&str, &[&str])] = &[
    ...
    (
        "sync",
        &["useSync", "useLiveQuery", "SyncProvider", "<Sync"],
    ),
    (
        "offline",
        &[
...
impl FeatureTreeShaker {
    ...
    pub fn new(config: &FeaturesConfig) -> Self {
        let enabled_features: HashSet<String> = config
            .enabled_features()
            .into_iter()
            .map(|s| s.to_string())
            .collect();
    ...
    pub fn tree_shake(&self, source: &str, file_path: &str) -> TreeShakeResult {
        ...
        let mut removed_indices: Vec<usize> = analysis
            .imports_to_remove
            .iter()
            .map(|i| i.line - 1)
            .collect();
...
function App() {
    ...
    fn test_get_feature_for_import() {
        ...
        assert_eq!(
            shaker.get_feature_for_import("dx/forms"),
            Some("forms".to_string())
        );
        assert_eq!(
            shaker.get_feature_for_import("@dx/query"),
    ...
    fn test_get_feature_for_api() {
        ...
        assert_eq!(
            shaker.get_feature_for_api("useForm"),
            Some("forms".to_string())
        );
        assert_eq!(
            shaker.get_feature_for_api("useQuery"),
            Some("query".to_string())
        );
...
mod property_tests {
    ...
    fn arbitrary_enabled_features() -> impl Strategy<Value = FeaturesConfig> {
        (
            ...
            .prop_map(
                |(forms, query, auth, sync, offline, a11y, i18n)| FeaturesConfig {
                    forms,
                    query,
                    auth,
                    sync,
...
mod usage_graph_tests {
    ...
    fn test_usage_graph_basic() {
        ...
        graph.add_reference(
            SymbolId::new("main", "App"),
            SymbolId::new("main", "helper"),
        );
