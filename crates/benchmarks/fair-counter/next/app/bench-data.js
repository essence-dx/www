...
export function dashboardRows() {
  ...
  return Array.from({ length: 1200 }, (_, index) => {
    ...
    return {
      ...
      plan: plans[number % plans.length],
      region: regions[number % regions.length],
      status,
      mrr: 400 + (number % 37) * 91,
      risk: (number * 7) % 100,
      search: `account ${number} ${plans[number % plans.length]} ${regions[number % regions.length]} ${status}`.toLowerCase(),
    };
  });
}
