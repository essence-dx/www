...
pub use style::{
    BinaryStyleBundle, StyleArtifactMetadata, StyleConfig, StyleDeliveryMode, StyleProcessor,
};
...
pub struct BuildConfig {
    ...
    /// Enable style processing (generated CSS by default, Binary Dawn by explicit mode)
...
impl BuildPipeline {
    ...
    pub fn build(&mut self) -> Result<BuildResult> {
        ...
        stats.build_time_ms = start_time
            .elapsed()
    ...
    fn process_styles(&mut self) -> Result<Vec<BuildArtifact>> {
        ...
        processor.compile_for_browser(&mut self.cache)
    }
