...
//! - `pages/user/[id].html` → `/user/:id` (dynamic)
//! - `pages/docs/[...slug].html` → `/docs/*` (catch-all)
//! - `pages/_layout.html` → Layout applied to siblings and children
...
impl FileSystemRouter {
    ...
    fn add_page(&mut self, page: &PageFile, project: &Project) -> DxResult<()> {
        ...
        if page.is_special {
            if page.route_path.contains("_error") || page.path.ends_with("error.tsx") {
                self.error_route = Some(route);
            } else if page.route_path.contains("_404") || page.path.ends_with("not-found.tsx") {
        ...
        if let Some(existing_route) = self.static_routes.get(&normalized_path) {
            return Err(DxError::DuplicateRoute {
                path: normalized_path.clone(),
                file1: existing_route.file_path.clone(),
    ...
    fn normalize_path(&self, path: &str) -> String {
        let mut normalized = strip_request_suffix(path).to_string();
        ...
        if self.trailing_slash && !normalized.ends_with('/') && normalized != "/" {
            normalized.push('/');
        } else {
    ...
    fn compute_binary_path(&self, source_path: &Path, project: &Project) -> PathBuf {
...
mod tests {
    ...
    fn create_test_route(path: &str) -> Route {
        Route {
            path: path.to_string(),
            file_path: PathBuf::from(format!("pages{path}.html")),
    ...
    fn test_static_route_matching() {
        ...
        router
            .static_routes
            .insert("/".to_string(), create_test_route("/"));
        router
            .static_routes
            .insert("/about".to_string(), create_test_route("/about"));
    ...
    fn test_catch_all_route_matching() {
        ...
        router
            .catch_all_routes
            .push(create_test_route("/docs/*slug"));

        let result = router.match_route("/docs/getting-started/intro");
        assert!(result.is_some());
        let route_match = result.unwrap();
        assert_eq!(
            route_match.params.get("slug"),
    ...
    #[test]
    fn route_matching_ignores_query_and_fragment_suffixes() {
        let mut router = FileSystemRouter {
            static_routes: HashMap::new(),
            dynamic_routes: Vec::new(),
            catch_all_routes: Vec::new(),
        ...
        router
            .static_routes
            .insert("/dashboard".to_string(), create_test_route("/dashboard"));
        router.dynamic_routes.push(create_test_route("/blog/:slug"));

        assert!(router.match_route("/dashboard?tab=launch#status").is_some());

        let route_match = router
            .match_route("/blog/launch%20notes?preview=true")
            .expect("dynamic route should ignore query and decode params");
        assert_eq!(
            route_match.params.get("slug"),
