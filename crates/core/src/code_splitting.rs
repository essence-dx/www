...
impl ChunkManifest {
    ...
    pub fn from_classification(result: &ClassificationResult) -> Self {
        ...
        let groups = [
            (
                HandlerGroup::Critical,
                "handlers_critical.dxb",
                &result.critical,
            ),
            (
                HandlerGroup::Interactive,
                "handlers_secondary.dxb",
                &result.interactive,
            ),
            (
                HandlerGroup::Submission,
                "handlers_submission.dxb",
                &result.submission,
            ),
            (
                HandlerGroup::Navigation,
                "handlers_navigation.dxb",
                &result.navigation,
            ),
...
mod tests {
    ...
    fn test_handler_classification_critical() {
        ...
        assert_eq!(
            HandlerClassifier::classify(&handler),
            HandlerGroup::Critical
        );
    }
    ...
    proptest! {
        ...
        fn prop_chunk_count_between_3_and_5(
            ...
            prop_assert!((1..=5).contains(&chunk_count));
        }
