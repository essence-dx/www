...
const PAYMENTS_PACKAGE_STATUS: &str = ".dx/forge/package-status.json";
const PAYMENTS_BILLING_WORKFLOW_RECEIPT: &str =
...
pub(super) fn forge_payments_package_metrics(
    ...
    let manifest_package_present = manifest
        .packages
        .iter()
        .any(|package| package.package_id == PAYMENTS_PACKAGE_ID);
    ...
    let Some(package_status) = read_optional_forge_json(
        root,
        PAYMENTS_PACKAGE_STATUS,
        "payments-package-status-invalid",
        &mut findings,
    ) else {
        if manifest_package_present
            || package_receipt_exists(root, PAYMENTS_BILLING_WORKFLOW_RECEIPT)
        {
            package_present = 1;
            missing_receipt = 1;
    ...
    let Some(visibility) = json_array_entries(&package_status, &["package_lane_visibility"])
        .into_iter()
        .find(|entry| json_text(entry, &["package_id"]) == Some(PAYMENTS_PACKAGE_ID))
    else {
        if manifest_package_present
            || package_receipt_exists(root, PAYMENTS_BILLING_WORKFLOW_RECEIPT)
        {
    ...
    let package_receipt_path = json_text(visibility, &["package_receipt_path"])
        .unwrap_or(PAYMENTS_BILLING_WORKFLOW_RECEIPT);
    ...
    if stale_receipt > 0 {
        findings.push(check_finding(
            ...
            format!("{PAYMENTS_OFFICIAL_NAME} package-status visibility is stale"),
            Some(PAYMENTS_PACKAGE_STATUS.to_string()),
            "Regenerate the Payments package-status row from the billing workflow receipt before claiming source freshness.",
        ));
    }
    if blocked_surfaces > 0 {
        findings.push(check_finding(
            ...
            format!("{PAYMENTS_OFFICIAL_NAME} has {blocked_surfaces} blocked selected surface(s)"),
            Some(PAYMENTS_PACKAGE_STATUS.to_string()),
            "Resolve the ADAPTER-BOUNDARY Payments app-owned Stripe credentials, Price IDs, webhook fulfillment, entitlement policy, and Checkout runtime before claiming the selected surface is release-ready.",
        ));
    }
    if unsupported_surfaces > 0 {
        findings.push(check_finding(
            ...
            format!(
                "{PAYMENTS_OFFICIAL_NAME} has {unsupported_surfaces} unsupported requested surface(s)"
            ),
            Some(PAYMENTS_PACKAGE_STATUS.to_string()),
            "Request only supported Payments surfaces or add a real upstream-backed Forge surface first.",
        ));
    }
    if hash_mismatches > 0 {
        findings.push(check_finding(
            ...
            format!(
                "{PAYMENTS_OFFICIAL_NAME} has {hash_mismatches} missing or stale hash-backed source file(s)"
            ),
            Some(PAYMENTS_PACKAGE_STATUS.to_string()),
            "Regenerate the Payments billing workflow receipt after reviewing the changed Checkout, webhook, and launch billing source files.",
        ));
    }
    if dx_style_compatibility_missing > 0 {
        findings.push(check_finding(
            ...
            format!("{PAYMENTS_OFFICIAL_NAME} is missing dx-style compatibility evidence"),
            Some(PAYMENTS_PACKAGE_STATUS.to_string()),
            "Regenerate the Payments dx-style compatibility row from the billing workflow receipt and verify token-surface markers without claiming live Stripe Checkout visual proof.",
        ));
    }
...
fn payments_metrics(
    ...
    vec![
        ...
        check_metric("payments_hash_manifest_present", hash_manifest_present),
        check_metric("payments_hash_mismatch", hash_mismatches),
        check_metric(
...
fn hash_project_file_sha256(root: &Path, relative_path: &str) -> Option<String> {
    let path = resolve_dx_check_relative_path(root, relative_path).or_else(|| {
        relative_path
...
fn missing_package_status_finding() -> DxCheckFinding {
    check_finding(
        ...
        format!("{PAYMENTS_OFFICIAL_NAME} is missing from package-status visibility"),
        Some(PAYMENTS_PACKAGE_STATUS.to_string()),
        "Regenerate .dx/forge/package-status.json so dx-check can report Payments visibility states.",
    )
}
...
fn missing_receipt_finding(receipt_path: &str) -> DxCheckFinding {
    check_finding(
        ...
        format!("{PAYMENTS_OFFICIAL_NAME} is missing its package-lane receipt"),
        Some(receipt_path.to_string()),
        "Regenerate or restore the Payments billing workflow receipt so dx-check can report source-owned package visibility.",
    )
}
...
mod tests {
    ...
    #[test]
    fn payments_package_metrics_reports_missing_dx_style_compatibility() {
        let dir = tempfile::tempdir().expect("tempdir");
        write_payments_receipt(dir.path());
        write_payments_package_status(dir.path(), false);

        let (missing_metrics, missing_findings) =
            forge_payments_package_metrics(dir.path(), &DxSourceManifest::default());

        assert_eq!(
            metric_value(&missing_metrics, "payments_package_present"),
            Some(1)
        );
        assert_eq!(
            metric_value(&missing_metrics, "payments_receipt_present"),
            Some(1)
        );
        assert_eq!(
            metric_value(&missing_metrics, "payments_dx_style_compatibility_present"),
            Some(0)
        );
        assert_eq!(
            metric_value(&missing_metrics, "payments_dx_style_compatibility_missing"),
            Some(1)
        );
        assert!(
            missing_findings
                .iter()
                .any(|finding| finding.code == "payments-missing-dx-style-compatibility")
        );

        write_payments_package_status(dir.path(), true);

        let (present_metrics, present_findings) =
            forge_payments_package_metrics(dir.path(), &DxSourceManifest::default());
        assert_eq!(
            metric_value(&present_metrics, "payments_dx_style_compatibility_present"),
            Some(1)
        );
        assert_eq!(
            metric_value(&present_metrics, "payments_dx_style_compatibility_missing"),
            Some(0)
        );
        assert!(
            !present_findings
    ...
    fn write_payments_receipt(root: &Path) {
        let receipt_path = root.join(PAYMENTS_BILLING_WORKFLOW_RECEIPT);
        fs::create_dir_all(receipt_path.parent().expect("receipt parent"))
            .expect("receipt directory");
        fs::write(
            receipt_path,
            serde_json::to_vec_pretty(&serde_json::json!({
                "schema": "dx.forge.payments.receipt",
                "official_package_name": PAYMENTS_OFFICIAL_NAME,
                "package_id": PAYMENTS_PACKAGE_ID
            }))
            .expect("receipt json"),
        )
    ...
    fn write_payments_package_status(root: &Path, include_dx_style: bool) {
        write_payments_package_status_with_refresh(root, include_dx_style, "current", 0, 0, None);
    }

    #[test]
    fn payments_hash_refresh_stale_helper_keeps_source_hash_clean() {
        let dir = tempfile::tempdir().expect("tempdir");
        let source_path = dir.path().join("examples/template/payments-status.tsx");
        fs::create_dir_all(source_path.parent().expect("source parent")).expect("source dir");
        fs::write(
            &source_path,
            b"export const stablePaymentsBillingWorkflow = true;\n",
        )
        .expect("write source");
        let source_hash =
            hash_project_file_sha256(dir.path(), "examples/template/payments-status.tsx")
                .expect("hash source");

        write_payments_receipt(dir.path());
        write_payments_package_status_with_refresh(
            dir.path(),
            true,
            "current",
            0,
        ...
        let (current_metrics, current_findings) =
            forge_payments_package_metrics(dir.path(), &manifest);
        assert_eq!(
            metric_value(&current_metrics, "payments_receipt_hash_refresh_current"),
            Some(1)
        );
        assert_eq!(
            metric_value(&current_metrics, "payments_receipt_hash_refresh_stale"),
            Some(0)
        );
        assert_eq!(
            metric_value(&current_metrics, "payments_receipt_hash_refresh_missing"),
            Some(0)
        );
        assert_eq!(
            metric_value(&current_metrics, "payments_hash_mismatch"),
            Some(0)
        );
        assert_eq!(
            metric_value(&current_metrics, "payments_receipt_stale"),
            Some(0)
        );
        assert!(
            !current_findings
                .iter()
                .any(|finding| finding.code == "payments-stale-receipt")
        );
        ...
        let (stale_metrics, stale_findings) = forge_payments_package_metrics(dir.path(), &manifest);
        assert_eq!(
            metric_value(&stale_metrics, "payments_receipt_hash_refresh_current"),
            Some(0)
        );
        assert_eq!(
            metric_value(&stale_metrics, "payments_receipt_hash_refresh_stale"),
            Some(1)
        );
        assert_eq!(
            metric_value(&stale_metrics, "payments_receipt_hash_refresh_missing"),
            Some(0)
        );
        assert_eq!(
            metric_value(&stale_metrics, "payments_hash_mismatch"),
            Some(0)
        );
        assert_eq!(
            metric_value(&stale_metrics, "payments_receipt_stale"),
            Some(1)
        );
        assert!(
            stale_findings
        ...
        assert!(
            !stale_findings
                .iter()
                .any(|finding| finding.code == "payments-hash-mismatch")
        );
    }

    fn write_payments_package_status_with_refresh(
        root: &Path,
        ...
        let package_status_path = root.join(PAYMENTS_PACKAGE_STATUS);
        ...
        let mut selected_surface = serde_json::json!({
            ...
            "status": "present",
            "receipt_path": PAYMENTS_BILLING_WORKFLOW_RECEIPT,
            "files": ["components/launch/payments-status.tsx"],
            "source_markers": [
        ...
        if let Some(source_hash) = source_hash {
            ...
            selected_surface["file_hashes"] = serde_json::json!({
                "examples/template/payments-status.tsx": source_hash
            });
        }
        ...
        if include_dx_style {
            visibility["dx_style_compatibility"] = serde_json::json!({
                ...
                "visible_surfaces": ["launch-billing-checkout-workflow"],
                "source_files": ["components/launch/payments-status.tsx"],
                "runtime_proof": false,
                "runtime_limitations": [
                    "SOURCE-ONLY: no live Stripe Checkout visual proof is claimed."
                ]
            });
        }
