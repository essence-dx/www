...
type UpdateHandler = Box<dyn Fn(&ModuleUpdate) + Send + Sync>;
type UpdateHandlerMap = HashMap<String, Vec<UpdateHandler>>;
...
pub struct HmrRuntime {
    ...
    /// Update callbacks
    update_handlers: UpdateHandlerMap,
    /// Pending updates
...
mod tests {
    ...
    fn test_module_graph() {
        ...
        graph.add_dependency("app.html", "button.tsx");

        let affected = graph.get_affected_modules("button.tsx");
        assert!(affected.contains(&"button.tsx".to_string()));
        assert!(affected.contains(&"app.html".to_string()));
    }
    ...
    fn test_hmr_runtime() {
        ...
        runtime.register_module("test.tsx", "abc123");
        runtime.accept("test.tsx");

        assert!(runtime.modules.get("test.tsx").unwrap().accept);
    }
}
