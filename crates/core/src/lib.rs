...
//! TSX/JSX parsing, delivery analysis, source render contracts, runtime planning,
//! and legacy binary-format experiments used by compatibility tests.
//!
//! ## Overview
//!
//! The dx-compiler analyzes TSX/JSX source files through a multi-stage pipeline:
//!
...
//! 5. **Code Generation** - Generate source-owned delivery/runtime contracts
//! 6. **Packing** - Package internal artifacts for experiments and tests
//!
//! ## Quick Start
//!
//! ```rust,ignore
...
pub mod delivery;
pub mod dev_server;
pub mod dx_parser; // DX format parser (.html, .tsx, .lyt files)
pub mod errors;
pub mod linker;
pub mod loader;
pub mod packer;
...
// Legacy/internal binary-format experiments.
pub mod binary_compiler;
...
pub struct CompileResult {
    ...
    /// Path to the generated internal HTIP artifact.
    ...
    /// Path to the adaptive delivery manifest
    pub delivery_manifest_path: PathBuf,
    /// Adaptive delivery plan selected by the compiler
    pub delivery_plan: delivery::DxDeliveryPlan,
...
/// Compile a TSX entry file to source-owned compiler artifacts.
///
/// This is the main entry point for the dx-compiler. It performs the complete
/// compilation pipeline from TSX source to generated delivery artifacts.
///
/// # Arguments
///
...
pub fn compile_tsx(entry: &Path, output: &Path, verbose: bool) -> Result<CompileResult> {
    ...
    // Step 6: Plan adaptive delivery modes
    ...
    if verbose {
        println!("  Delivery mode: {}", delivery_plan.primary_mode.as_str());
    }

    // Step 7: Write HTIP to disk
    let htip_path = output.join("app.htip");
    std::fs::write(&htip_path, &htip_stream)?;

    // Step 8: Generate templates.json
    ...
    // Step 9: Generate Rust code based on runtime variant
    ...
    // Step 10: Pack into .dxb (using pack_dxb_htip for compatibility)
