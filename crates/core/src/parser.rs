...
fn parse_with_oxc(
    ...
    if verbose {
        println!("    [OXC] Validating TSX syntax: {}", path.display());
    }
...
pub fn tree_shake(modules: Vec<ParsedModule>, verbose: bool) -> Result<Vec<ParsedModule>> {
                ...
                for spec in &import.specifiers {
                    ...
                    let is_used = module.components.iter().any(|c| {
                        c.jsx_body.contains(local_name)
                            ...
                            || c.hooks
                                .iter()
                                .any(|h| h.args.iter().any(|a| a.contains(local_name)))
                    });
    ...
    if verbose {
        ...
        println!(
            "    ✓ {} imports remaining after tree shaking",
            total_imports
        );
    }
...
fn extract_component_from_export_default(
    ...
    match &export.declaration {
        ExportDefaultDeclarationKind::FunctionDeclaration(func) => {
            ...
            Some(create_component_from_function_body(
                &name,
                func.r#async,
                source,
            ))
...
fn extract_component_from_var_decl(
    ...
    let is_async = decl.init.as_ref().is_some_and(|init| {
        matches!(
            init,
