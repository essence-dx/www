...
impl BuildCache {
    ...
    pub async fn check(&self, path: &Path) -> DxResult<Option<CacheEntry>> {
        if let Some(entry) = self.entries.get(path) {
            ...
            // Check if output file exists and still matches the cached artifact shape.
            let Ok(output_metadata) = tokio::fs::metadata(&entry.output_path).await else {
                return Ok(None);
            };
            if output_metadata.len() as usize != entry.size {
                return Ok(None);
            }
    ...
    pub async fn set(
        ...
        output_path: PathBuf,
        output_size: usize,
        compiled: crate::build::CompiledComponent,
    ) {
...
mod tests {
    use super::*;
    use crate::build::CompiledComponent;
    use crate::parser::{ParsedComponent, ParsedTemplate};
    ...
    fn test_cache_invalidate() {
        ...
        let entry = CacheEntry {
            source_path: PathBuf::from("/test/file.html"),
        ...
        cache.invalidate(Path::new("/test/file.html"));
        assert_eq!(cache.entries.len(), 0);
        assert!(cache.dirty);
    }
    ...
    fn test_cache_entry_to_binary_object() {
        let entry = CacheEntry {
            source_path: PathBuf::from("/test/page.html"),
            ...
            component_type: 0x01,
            size: 500,
            dependencies: vec![PathBuf::from("/test/shared.tsx")],
        };
    ...
    #[tokio::test]
    async fn test_cache_set_records_output_artifact_shape() {
        let temp = tempfile::tempdir().expect("temp cache");
        let source_path = temp.path().join("page.html");
        let output_path = temp.path().join("out/page.dxob");
        tokio::fs::write(&source_path, "<template>Hello</template>")
            .await
            .expect("source");
        ...
        cache
            .set(source_path.clone(), output_path.clone(), 4096, compiled)
            .await;

        let entry = cache.entries.get(&source_path).expect("cache entry");
        assert_eq!(entry.output_path, output_path);
        assert_eq!(entry.size, 4096);
        assert_eq!(entry.output_hash, "compiled-hash");
        assert!(cache.dirty);
    }

    #[tokio::test]
    async fn test_cache_check_rejects_stale_output_size() {
        let temp = tempfile::tempdir().expect("temp cache");
        let source_path = temp.path().join("page.html");
        let output_path = temp.path().join("out/page.dxob");
        tokio::fs::create_dir_all(output_path.parent().expect("output parent"))
            .await
            .expect("output dir");
        tokio::fs::write(&source_path, "<template>Hello</template>")
            .await
            .expect("source");
        tokio::fs::write(&output_path, [1_u8, 2, 3])
            .await
            .expect("output");
        ...
        let mtime = source_metadata
            ...
            .unwrap_or(0);
        let source_hash = blake3::hash(&tokio::fs::read(&source_path).await.expect("source bytes"))
            .to_hex()[..16]
            .to_string();
        ...
        cache.entries.insert(
            ...
            CacheEntry {
                ...
                output_path,
                source_hash,
                output_hash: "compiled-hash".to_string(),
                mtime,
                component_type: 0x01,
                size: 4096,
                dependencies: Vec::new(),
        ...
        let cached = cache.check(&source_path).await.expect("cache check");
