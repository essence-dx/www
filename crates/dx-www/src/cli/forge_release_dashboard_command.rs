...
use super::build_forge_release_dashboard_report;
use super::forge_error;
use super::forge_release_dashboard::{
    forge_release_dashboard_failure_summary, forge_release_dashboard_markdown,
};
...
pub(super) fn run_forge_release_dashboard(cwd: &Path, args: &[String]) -> DxResult<()> {
        ...
        match args[index].as_str() {
            ...
            value if value.starts_with('-') => {
                return Err(DxError::ConfigValidationError {
                    message: format!("Unknown forge release-dashboard option: {value}"),
                    field: Some("forge release-dashboard".to_string()),
                });
            }
            value => {
                if project.is_some() {
                    return Err(DxError::ConfigValidationError {
                        message: format!("Unexpected forge release-dashboard path: {value}"),
                        field: Some("project".to_string()),
                    });
                }
                project = Some(resolve_cli_path(cwd, value));
    ...
    let history =
        history.unwrap_or_else(|| cwd.join("benchmarks/reports/vertical-proof-history/index.json"));
    let route_comparison = route_comparison
        .unwrap_or_else(|| cwd.join("benchmarks/reports/forge-public-route-comparison.json"));

    let report = build_forge_release_dashboard_report(
        &project,
        &ci_artifacts,
        &pages,
        &history,
    ...
    if report.score < fail_under {
        return Err(DxError::ConfigValidationError {
            message: format!(
                "DX Forge release-dashboard score {} is below fail-under threshold {fail_under}",
                report.score
            ),
            field: Some("fail-under".to_string()),
        });
    ...
    if !report.passed {
        return Err(DxError::InternalError {
            message: forge_release_dashboard_failure_summary(&report),
        });
    }
