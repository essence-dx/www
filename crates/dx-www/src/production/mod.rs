...
type ProductionResult<T> = Result<T, Box<DxError>>;
...
/// Error returned when parsing a deployment target fails.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ParseDeploymentTargetError;

impl std::fmt::Display for ParseDeploymentTargetError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("unknown deployment target")
    }
}

impl std::error::Error for ParseDeploymentTargetError {}

impl std::str::FromStr for DeploymentTarget {
    type Err = ParseDeploymentTargetError;
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        .ok_or(ParseDeploymentTargetError)
    }
}
...
impl ProductionBuilder {
    ...
    pub fn build(&mut self, source_dir: &Path) -> DxResult<BuildResult> {
        self.build_inner(source_dir).map_err(|error| *error)
    }

    fn build_inner(&mut self, source_dir: &Path) -> ProductionResult<BuildResult> {
    ...
    fn collect_files(&self, dir: &Path) -> ProductionResult<Vec<PathBuf>> {
    ...
    fn process_file(&mut self, path: &Path) -> ProductionResult<()> {
        ...
        let (output_content, source_map) = match asset_type {
            AssetType::JavaScript if self.config.minify => self.minify_javascript(&content, path),
    ...
    /// Minify JavaScript.
    fn minify_javascript(&self, content: &[u8], path: &Path) -> (Vec<u8>, Option<SourceMap>) {
        ...
        (result.into_bytes(), source_map)
    }

    /// Minify CSS.
    fn minify_css(&self, content: &[u8], path: &Path) -> (Vec<u8>, Option<SourceMap>) {
        let source = String::from_utf8_lossy(content);
    ...
    fn write_source_maps(&self) -> ProductionResult<()> {
...
mod tests {
    ...
    #[test]
    fn test_deployment_target_from_str_trait() {
        assert_eq!(
            "static".parse::<DeploymentTarget>(),
            Ok(DeploymentTarget::Static)
        );
        assert_eq!(
            "node".parse::<DeploymentTarget>(),
            Ok(DeploymentTarget::Node)
        );
        assert_eq!(
            "cloudflare".parse::<DeploymentTarget>(),
            Ok(DeploymentTarget::CloudflareWorkers)
        );
        assert_eq!(
            "vercel".parse::<DeploymentTarget>(),
            Ok(DeploymentTarget::Vercel)
        );
        assert_eq!(
            "lambda".parse::<DeploymentTarget>(),
            Ok(DeploymentTarget::Lambda)
        );
        assert_eq!(
            "docker".parse::<DeploymentTarget>(),
            Ok(DeploymentTarget::Docker)
        );
        assert!("unknown".parse::<DeploymentTarget>().is_err());
    }
    ...
    fn test_deployment_target_name() {
        ...
        assert_eq!(
            DeploymentTarget::CloudflareWorkers.name(),
            "cloudflare-workers"
        );
    }
