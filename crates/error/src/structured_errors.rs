...
impl std::error::Error for DxError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            DxError::Auth { source, .. } => source
                .as_ref()
                .map(|e| e.as_ref() as &(dyn std::error::Error + 'static)),
            DxError::Database { source, .. } => source
                .as_ref()
                .map(|e| e.as_ref() as &(dyn std::error::Error + 'static)),
            DxError::Sync { source, .. } => source
                .as_ref()
                .map(|e| e.as_ref() as &(dyn std::error::Error + 'static)),
            DxError::Internal { source, .. } => source
                .as_ref()
                .map(|e| e.as_ref() as &(dyn std::error::Error + 'static)),
            _ => None,
        }
...
mod tests {
    ...
    fn test_config_error_with_suggestion() {
        ...
        assert_eq!(
            ...
            Some("Set DATABASE_URL environment variable")
        );
    }
    ...
    fn test_all_auth_codes_have_suggestions() {
        ...
        for code in codes {
            assert!(
                code.recovery_suggestion().is_some(),
