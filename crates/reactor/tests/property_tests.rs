...
fn prop_opcode_uniqueness() {
    ...
    assert_eq!(
        all_opcodes.len(),
        seen_values.len(),
        "All opcodes should have unique values"
    );
}
...
fn prop_header_parsing_boundary() {
    ...
    let buffer_7 = [0u8; 7];
    assert!(
        HbtpHeader::from_bytes(&buffer_7).is_none(),
        "7 bytes should fail"
    );
    ...
    let buffer_8 = [0u8; 8];
    assert!(
        HbtpHeader::from_bytes(&buffer_8).is_some(),
        "8 bytes should succeed"
    );

    // Empty buffer - should fail
    let empty: [u8; 0] = [];
    assert!(
        HbtpHeader::from_bytes(&empty).is_none(),
        "empty buffer should fail"
    );
}
...
fn prop_flag_composition_combinations() {
    ...
    for i in 0u8..16 {
        ...
        assert_eq!(
            flags.contains(HbtpFlags::EXPECTS_RESPONSE),
            expects_response
        );
...
fn prop_timing_header_format() {
    ...
    assert!(
        numeric >= 1.0,
        "Duration should be at least 1ms, got {}",
        numeric
    );
}
...
fn prop_rate_limit_independent_counters() {
    ...
    assert_eq!(
        get_thread_rate_count(),
        5,
        "Main thread count should be unchanged"
    );
}
