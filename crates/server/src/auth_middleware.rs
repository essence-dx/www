...
pub async fn handle_login(
    ...
    if !req.email.contains('@') || req.password.len() < 8 {
        ...
        return Err((
            ...
            Json(AuthErrorResponse::invalid_credentials()),
        ));
    }
...
pub async fn handle_refresh(
    ...
    let refresh_token = AuthToken::from_base64(&req.refresh_token).map_err(|_| {
        ...
        (
            StatusCode::UNAUTHORIZED,
            Json(AuthErrorResponse::refresh_token_invalid()),
        )
    })?;
    ...
    if refresh_token.typ != TokenType::Refresh {
        ...
        return Err((
            StatusCode::UNAUTHORIZED,
            Json(AuthErrorResponse::token_type_mismatch()),
        ));
    }

    // Check if token is within grace period or still valid
    let is_within_grace = auth_state
        .token_generator
        .is_within_grace_period(&refresh_token);
    ...
    if !is_valid && !is_within_grace {
        ...
        return Err((
            StatusCode::UNAUTHORIZED,
            Json(AuthErrorResponse::refresh_token_expired()),
        ));
    }
    ...
    auth_state
        ...
        .map_err(|_| {
            tracing::warn!(
                "Invalid refresh token signature for user: {}",
                refresh_token.sub
            );
            (
                StatusCode::UNAUTHORIZED,
    ...
    if let Ok(true) = credential_store.is_token_revoked(&refresh_token.jti).await {
        ...
        return Err((
            StatusCode::UNAUTHORIZED,
            Json(AuthErrorResponse::token_revoked()),
        ));
    }
...
pub async fn verify_token_middleware(
    ...
    let auth_header = req
        ...
        .ok_or_else(|| {
            (
                StatusCode::UNAUTHORIZED,
                Json(AuthErrorResponse::missing_token()),
            )
        })?;
    ...
    let token = AuthToken::from_base64(token_str).map_err(|_| {
        ...
        (
            StatusCode::UNAUTHORIZED,
            Json(AuthErrorResponse::token_invalid()),
        )
    })?;
