...
impl CsrfManager {
    ...
    pub fn validate_at(
        ...
        let timestamp_bytes: [u8; 8] = token_bytes[16..24]
            .try_into()
...
mod tests {
    ...
    fn test_csrf_token_wrong_session() {
        ...
        assert_eq!(
            manager.validate(&token.value, "session456"),
    ...
    fn test_csrf_token_expired() {
        ...
        // Should be expired
        assert_eq!(
            manager.validate(&token.value, "session123"),
    ...
    fn test_csrf_token_invalid() {
        ...
        // Invalid token
        assert_eq!(
            manager.validate("invalid_token", "session123"),
            Err(CsrfError::Invalid)
        );
    }
    ...
    fn test_extract_csrf_from_form_body() {
        ...
        assert_eq!(
            extract_csrf_from_form_body(body),
    ...
    fn test_generate_csrf_hidden_field() {
        ...
        assert_eq!(
            html,
            r#"<input type="hidden" name="_csrf" value="token123">"#
        );
    ...
    fn test_csrf_validate_at_specific_time() {
        ...
        assert!(
            manager
                .validate_at(&token.value, "session123", token.created_at)
                .is_ok()
        );
        ...
        assert!(
            manager
                .validate_at(&token.value, "session123", just_before)
                .is_ok()
        );
        ...
        assert_eq!(
            manager.validate_at(&token.value, "session123", after),
            Err(CsrfError::Expired)
        );
    }
