            ...
            match limiter.check(&ip, category).await {
                Ok(RateLimitResult::Allowed { remaining }) => {
                    ...
                    if let Ok(limit_val) = HeaderValue::from_str(&limit.max_requests.to_string()) {
                        response
                            .headers_mut()
                            .insert("x-ratelimit-limit", limit_val);
                    }
                    if let Ok(remaining_val) = HeaderValue::from_str(&remaining.to_string()) {
                        response
                            .headers_mut()
                            .insert("x-ratelimit-remaining", remaining_val);
                    }
...
mod tests {
    ...
    async fn test_rate_limiter_allows_under_limit() {
        ...
        for i in 0..10 {
            let result = limiter
                .check("192.168.1.1", EndpointCategory::Auth)
                .await
                .unwrap();
            match result {
    ...
    async fn test_rate_limiter_different_ips() {
        ...
        let result = limiter
            .check("192.168.1.2", EndpointCategory::Auth)
            .await
            .unwrap();
    ...
    async fn test_rate_limiter_different_categories() {
        ...
        let result = limiter
            .check("192.168.1.1", EndpointCategory::Api)
            .await
            .unwrap();
        assert!(matches!(result, RateLimitResult::Allowed { .. }));
    }
