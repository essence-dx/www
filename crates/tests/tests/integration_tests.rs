...
fn test_hello_world_compilation() {
    ...
    assert!(
        compile_result.htip_path.exists(),
        "HTIP stream should exist"
    );
...
fn test_counter_app_compilation() {
    ...
    assert!(
        compile_result.templates_path.exists(),
...
///
/// Verifies that SSR-capable components compile correctly and the browser runtime
...
fn test_ssr_hydration_compilation() {
    ...
    // Verify the DXB package exists (for browser WASM runtime)
    let dxb_path = output.join("app.dxb");
    assert!(
        dxb_path.exists(),
        "DXB package should exist for browser WASM runtime"
    );
}
...
fn test_can_compile() {
    ...
    assert!(
        dx_www_compiler::can_compile(&valid_entry),
        "Valid TSX should be compilable"
    );
