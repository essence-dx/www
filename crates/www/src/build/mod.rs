//! # Build Pipeline
//!
//! This module implements the build pipeline for DX WWW projects. The current
//! public build path is source-owned route output, server data, generated CSS,
//! copied public assets, manifests, and receipts. Legacy object builders remain
//! available for compatibility paths.
//!
//! ## Build Process
//!
//! 1. Discover app routes, route handlers, styles, content, and public assets.
//! 2. Build source-owned route outputs and server-data routes.
//! 3. Compile styles through dx-style generated CSS.
//! 4. Copy public assets and emit manifests.
//! 5. Write build/check receipts and compatibility artifacts where required.
...
pub use binary::{BinaryObjectBuilder, DxobHeader, DxobReader, DxobSection};
pub use cache::{BuildCache, CacheEntry};
pub use manifest::{ManifestAsset, ManifestRoute, RouteManifest};
pub use optimize::{DependencyBundler, OptimizeOptions, Optimizer};
pub use script::ScriptCompiler;
pub use source_engine::{
    ContentDocsFreshnessStatus, SourceBuildAdapter, SourceBuildAsset,
    SourceBuildEcmascriptAnalysis, SourceBuildEcmascriptCompatibilityReference,
    SourceBuildEcmascriptDirective, SourceBuildEcmascriptDynamicImport,
    SourceBuildEcmascriptOutputModel, SourceBuildEcmascriptRuntimeBoundaries, SourceBuildEngine,
    SourceBuildImageMetadata, SourceBuildImageOptimization, SourceBuildImageRouteReference,
    SourceBuildImport, SourceBuildManifest, SourceBuildModuleChunk, SourceBuildModuleDependency,
    SourceBuildOptions, SourceBuildReceipt, SourceBuildReport, SourceBuildRoute,
    SourceBuildRouteOutput, SourceBuildServerDataRoute, SourceBuildStyle,
    SourceBuildStyleAssetReference, SourceBuildStyleImport, SourceBuildSummary,
    SourceBuildUpstream, evaluate_content_docs_hashes, evaluate_content_docs_receipt_freshness,
};
...
impl BuildPipeline {
    ...
    pub async fn build_incremental(
        ...
        for page in &project.pages {
            if !affected.contains(&page.path) {
                if let Some(cached) = self.cached_binary_object(&page.path, project).await? {
                    stats.files_cached += 1;
                    stats.total_size += cached.size;
                    binary_objects.push(cached);
                }
            }
        }
    ...
    async fn compile_component(
        ...
        let _permit = self
            ...
            .acquire()
            .await
            .map_err(|_| DxError::InternalError {
    ...
    /// Return a previously built artifact when the cache can prove it is still fresh.
    async fn cached_binary_object(
        &self,
        path: &PathBuf,
        project: &Project,
    ) -> DxResult<Option<BinaryObject>> {
        if let Some(cached) = self.cache.check(path).await? {
            return Ok(Some(cached.to_binary_object()));
        }

        let compiled = self.compiled.get(path).map(|entry| entry.value().clone());
        Ok(compiled.map(|compiled| self.binary_object_from_compiled(path, project, &compiled)))
    }

    fn binary_object_from_compiled(
        &self,
        path: &Path,
        project: &Project,
        ...
        BinaryObject {
            ...
            component_type: compiled.parsed.component_type,
        }
    }
    ...
    fn generate_manifest(
        ...
        RouteManifest::generate(project, binary_objects, assets)
    }

    /// Compute output path for a component.
    fn compute_output_path(&self, source: &Path, project: &Project) -> PathBuf {
