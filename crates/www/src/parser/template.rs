...
impl TemplateParser {
    ...
    fn parse_directive_type(&self, content: &str) -> TemplateDirective {
        ...
        } else if let Some(promise) = content.strip_prefix("await ") {
            TemplateDirective::Await {
                promise: promise.trim().to_string(),
                then_var: None,
                catch_var: None,
            }
        } else if let Some(expression) = content.strip_prefix("key ") {
    ...
    fn parse_attributes(
        ...
        while pos < chars.len() && chars[pos] != '>' && chars[pos] != '/' {
            ...
            if name.starts_with("on") || name.starts_with("@") {
                let event_name = if let Some(event) = name.strip_prefix('@') {
                    event.to_string()
...
mod tests {
    ...
    fn test_parse_simple_template() {
        ...
        let result = parser.parse(source, &PathBuf::from("test.html"));
        assert!(result.is_ok());
