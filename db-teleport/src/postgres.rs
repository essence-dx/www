...
impl DbTeleport {
    ...
    pub async fn with_notify_listener(pool: Pool, channel: &str) -> Result<Self> {
        ...
        tokio::spawn(async move {
            info!(
                "Starting DbTeleport notification listener for channel: {}",
                channel
            );
        ...
        F: FnOnce(Vec<tokio_postgres::Row>) -> Result<Vec<u8>>,
    {
        ...
        if let Some(cached) = self.get_cached(query_id, params_hash) {
            debug!(
                "Cache hit for query '{}' with params_hash {}",
        ...
        debug!(
            "Cache miss for query '{}' with params_hash {}",
            query_id, params_hash
        );
        ...
        F: FnOnce(Vec<tokio_postgres::Row>) -> Result<Vec<u8>>,
    {
        ...
        self.cache
            .set_cached_with_ttl(query_id, params_hash, data.clone(), ttl);
