...
Purpose: Zero-copy serialization for efficient data transfer between server and browser WASM runtime.
```rust
// SAFETY: Teleportable trait requires:
// 1. #[repr(C)] for stable memory layout // 2. No pointers or references (Copy types only)
// 3. Identical layout on server and browser WASM runtime pub unsafe trait Teleportable: Copy + 'static { ... }
```
...
### browser/src/allocator.rs

Purpose: Custom allocator for the browser WASM runtime to minimize binary size. Safety Invariants: -Allocator operates within WASM linear memory bounds -Alignment requirements are respected -No double-free or use-after-free
...
## Forge Import Evidence

Forge package import follows the [Forge import security model](forge-import-security-model.md).
The modeled import-review ecosystems are npm, pip, cargo, Go, JSR, pub, Maven,
NuGet, Composer, RubyGems, SwiftPM, Hex, and CRAN; this is not universal
package compatibility or package-manager execution.
`.dx/forge/package-status.json` is a read model for dashboards and editor
panels, not an authority artifact. Materialization claims must agree across the
source manifest, package lock, receipts, trust policy when present, remote status
when claimed, and current materialized file bytes.

BLAKE3 package and file integrity is used for package identity and registry
content verification. SHA-256 receipt freshness is used for package/dashboard
receipts and project-file drift checks. License and advisory metadata remains
declaration-only unless a receipt explicitly marks the data reviewed.
