...
use std::path::Path;
fn compile_app() -> anyhow::Result<CompileResult> { let entry = Path::new("app/page.tsx");
...
use std::path::Path;
fn check_complexity() -> anyhow::Result<()> { let entry = Path::new("app/page.tsx");
...
use std::path::Path;
fn main() { let result = compile_tsx( Path::new("app/page.tsx"), Path::new("dist"), false, );
...
dx-www-browser = { version = "0.1", features = ["wasm"] }
```
