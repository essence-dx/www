//! # Component Parser
//!
//! This module implements the parser for `.html` (page) and `.tsx` (component) files.
...
pub enum ComponentType {
    /// A page component (.html)
    Page,
    /// A reusable component (.tsx)
    Component,
    /// A layout component (_layout.html)
    Layout,
}
...
/// Parser for `.html` and `.tsx` component files.
#[derive(Debug)]
...
impl ComponentParser {
    ...
    fn extract_script(&self, content: &str, path: &Path) -> DxResult<Option<ParsedScript>> {
        if let Some(captures) = self.section_regex.script.captures(content) {
            ...
            let language =
                ScriptLanguage::parse(lang_str).ok_or_else(|| DxError::InvalidScriptLanguage {
...
"#;
    ...
    fn test_parse_component() {
        ...
        let result = parser.parse(Path::new("test.html"), SAMPLE_COMPONENT);
        assert!(result.is_ok());
    ...
    fn test_component_type_from_path() {
        assert_eq!(
            ComponentType::from_path(Path::new("about.html")),
            ComponentType::Page
        );
        assert_eq!(
            ComponentType::from_path(Path::new("Button.tsx")),
            ComponentType::Component
        );
        assert_eq!(
            ComponentType::from_path(Path::new("_layout.html")),
            ComponentType::Layout
        );
    }
...
"#;
        let result = parser.parse(Path::new("test.html"), content);
