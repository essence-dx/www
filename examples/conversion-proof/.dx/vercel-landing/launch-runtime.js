(() => {
  ...
  const fumadocsPages = [
    {
      ...
      peers: ["API reference", "LLM handoff"],
    ...
    },
    {
      ...
      peers: ["Launch overview", "LLM handoff"],
  ...
  function bindPayment() {
    const card = $('[data-dx-component="stripe-checkout-contact-proof"][data-dx-package="payments/stripe-js"]');
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        setPaymentResult(
          response.ok ? "posted" : "placeholder",
          response.ok
            ? `Checkout POST accepted: ${payload.status ?? payload.kind ?? "placeholder"}.`
            : `Checkout POST returned ${response.status}; local placeholder receipt prepared.`,
          receiptId,
        );
      } catch {
        ...
        setPaymentResult(
          "placeholder",
          `Checkout route unavailable; safe placeholder POST receipt ${receiptId} is visible until Stripe config exists.`,
          receiptId,
        );
      }
    });
  ...
  function bindFumadocsDocs() {
    const card = $('[data-dx-component="fumadocs-docs-navigation-proof"]');
  ...
  function bindApiActions() {
    const trpcProof = $("#trpc-runtime-proof");
    const trpcProofStatus = $("#trpc-proof-status");

    function setTrpcProof(message, status, requestId) {
      if (!trpcProof || !trpcProofStatus) return;
      trpcProof.dataset.trpcMutationState = status;
      trpcProofStatus.dataset.apiMutation = status;
      trpcProofStatus.dataset.trpcMutationState = status;
      trpcProofStatus.dataset.trpcRequestId = requestId;
      trpcProofStatus.textContent = message;
      trpcProofStatus.dataset.state = status === "error" ? "error" : "ok";
    }

    $("#trpc-health-check")?.addEventListener("click", async () => {
      setTrpcProof("Checking /api/trpc/health...", "pending", "pending");
      try {
    ...
    $("#trpc-launch-event")?.addEventListener("click", async () => {
      ...
      setTrpcProof("Recording safe local launchEvent validation...", "pending", requestId);
      try {
        const response = await fetch("/api/trpc/health", {
          method: "POST",
          headers: { "content-type": "application/json" },
