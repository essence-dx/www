...
fn bench_individual_optimizations(c: &mut Criterion) {
    ...
    group.bench_function("strip_urls", |b| {
        b.iter(|| black_box(markdown::strip_urls(black_box(&content))));
    });

    // Image stripping
    group.bench_function("strip_images", |b| {
        b.iter(|| black_box(markdown::strip_images(black_box(&content))));
    });

    // Badge stripping
    group.bench_function("strip_badges", |b| {
        b.iter(|| black_box(markdown::strip_badges(black_box(&content))));
    });
