...
use markdown::convert::{doc_to_human, llm_to_machine};
...
fn main() {
    println!(
        "\n{} {}",
        WRENCH,
        style("Fixing Specific Errored Files").bold().cyan()
    );
    ...
    let errored_files: Vec<PathBuf> = errored_paths
        .iter()
        .map(|p| dx_root.join(p))
        .filter(|p| p.exists())
        .collect();
