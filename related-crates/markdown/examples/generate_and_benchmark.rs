use markdown::convert::{human_to_llm, human_to_machine, machine_to_llm};
use std::fs;
...
fn main() {
    ...
    let avg_serialize = serialize_times
        .iter()
        .sum::<std::time::Duration>()
        .as_nanos()
        / serialize_times.len() as u128;
    let avg_deserialize = deserialize_times
        .iter()
        .sum::<std::time::Duration>()
        .as_nanos()
    ...
    println!("\nPerformance:");
    println!(
        "  Serialize (human→machine):   {:.2} µs avg",
        avg_serialize as f64 / 1000.0
    );
    println!(
        "  Deserialize (machine→llm):   {:.2} µs avg",
        avg_deserialize as f64 / 1000.0
    );
    println!(
        "  Round-trip:                  {:.2} µs avg",
