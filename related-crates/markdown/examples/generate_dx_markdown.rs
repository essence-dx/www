use markdown::convert::{llm_to_human, llm_to_machine};
...
fn _main_disabled() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for md_file in &markdown_files {
        ...
        let human_content = match llm_to_human(&result.output) {
            ...
            Err(e) => {
                eprintln!(
                    "✗ Failed to convert to human format {}: {}",
                    md_file.display(),
                    e
                );
                failed += 1;
        ...
        if let Err(e) = fs::write(&human_path, &human_content) {
            eprintln!(
                "✗ Failed to write Human format to {}: {}",
                human_path.display(),
                e
            );
            failed += 1;
        ...
        if let Err(e) = fs::write(&machine_path, &machine_content) {
            eprintln!(
                "✗ Failed to write Machine format to {}: {}",
