...
fn main() {
    ...
    // Calculate stats
    let avg_serialize_ns = serialize_times
        .iter()
        .sum::<std::time::Duration>()
        .as_nanos()
        / serialize_times.len() as u128;
    let avg_deserialize_ns = deserialize_times
        .iter()
        .sum::<std::time::Duration>()
        .as_nanos()
    ...
    println!(
        "  Serialize:   {:.2} µs avg",
        avg_serialize_ns as f64 / 1000.0
    );
    println!(
        "  Deserialize: {:.2} µs avg",
        avg_deserialize_ns as f64 / 1000.0
    );
    println!(
        "  Round-trip:  {:.2} µs avg",
