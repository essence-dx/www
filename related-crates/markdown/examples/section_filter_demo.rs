...
use markdown::{SectionFilterConfig, analyze_sections, filter_sections};
use std::fs;
...
fn main() {
    ...
    println!(
        "   Full README.md with ALL content: {} bytes\n",
        readme.len()
    );
    ...
    for section in &sections {
        ...
        println!(
            "   • {} ({}, ~{} tokens)",
            section.header, type_str, section.estimated_tokens
        );
    }
