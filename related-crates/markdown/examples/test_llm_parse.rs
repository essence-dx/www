...
fn main() {
    ...
    println!("\nNow attempting machine conversion...");
    use markdown::convert::llm_to_machine;
    match std::panic::catch_unwind(|| llm_to_machine(&llm)) {
        Ok(Ok(binary)) => {
            println!("✓ Machine conversion successful: {} bytes", binary.len());
        }
        Ok(Err(e)) => {
            eprintln!("✗ Machine conversion error: {}", e);
        }
        Err(e) => {
            eprintln!("✗ Machine conversion panicked: {:?}", e);
        }
    }
}
