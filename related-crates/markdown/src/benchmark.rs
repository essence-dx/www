...
impl BenchmarkTokenizer {
    ...
    pub fn is_native(&self) -> bool {
        matches!(
            self,
            Self::OpenAiCl100k | Self::OpenAiO200k | Self::OpenAiP50k
        )
    }
...
impl BenchmarkRunner {
    ...
    pub fn benchmark_content(
        ...
        for bench_tok in &self.tokenizers {
            ...
            tokenizer_results.push(TokenizerResult::new(
                *bench_tok,
                tokens_before,
                tokens_after,
            ));
    ...
    pub fn benchmark_suite(
        ...
        for bench_tok in &self.tokenizers {
            ...
            for file_result in &file_results {
                if let Some(tok_result) = file_result
                    .tokenizer_results
                    .iter()
                    .find(|r| r.tokenizer == *bench_tok)
                {
        ...
        for category in [
            ...
            let category_files: Vec<_> = file_results
                .iter()
                .filter(|f| f.category == category)
                .collect();

            if !category_files.is_empty() {
                let avg_savings: f64 = category_files
                    .iter()
                    .map(|f| f.average_savings())
                    .sum::<f64>()
...
mod tests {
    ...
    fn test_benchmark_tokenizer_from_str() {
        assert_eq!(
            BenchmarkTokenizer::from_str("cl100k"),
            Some(BenchmarkTokenizer::OpenAiCl100k)
        );
        assert_eq!(
            BenchmarkTokenizer::from_str("claude"),
            Some(BenchmarkTokenizer::Claude)
        );
