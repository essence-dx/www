...
impl ParseErrorKind {
    ...
    pub fn into_parse_error(self) -> ParseError {
        match self {
            ...
            Self::InvalidHeaderLevel { level, line } => ParseError::new(
                format!("invalid header level {level} (must be 1-6)"),
                line,
                1,
            )
            ...
            Self::RecursionLimitExceeded { depth, max } => ParseError::new(
                format!("recursion depth {depth} exceeds maximum {max}"),
                1,
                1,
            )
