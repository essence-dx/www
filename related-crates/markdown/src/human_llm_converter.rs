...
mod tests {
    ...
    fn test_parse_compact_table_simple() {
        ...
        assert!(
            result.is_some(),
            "parse_compact_table should return Some for valid table"
        );
        let table = result.unwrap();
        assert!(table.contains("+"), "Table should contain + characters");
        assert!(
            table.contains("Metric"),
            "Table should contain header 'Metric'"
        );
    ...
    fn test_md_to_human_format_with_table() {
        ...
        assert!(
            result.contains("+"),
            "Result should contain ASCII table with + characters"
        );
        assert!(
            result.contains("Metric"),
            "Result should contain header 'Metric'"
        );
    ...
    fn test_clean_variable_references() {
        ...
        assert_eq!(
            cleaned,
            "Performance: 10.59x faster than Bun with SharedArrayBuffer support"
        );
    }
