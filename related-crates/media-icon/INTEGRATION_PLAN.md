...
### Evidence Boundary (Native Rust)
- JSON icon packs remain the source of truth.
- Generated `.machine` files are advisory local caches.
- Local receipt writers can record startup, body-resolution, query-latency, and
  existing-cache readiness data when explicitly invoked.
