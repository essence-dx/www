...
fn load_engine(include_pack_body_cache: bool) -> anyhow::Result<()> {
    let mut engine_lock = ENGINE.lock().unwrap();

    let should_load = engine_lock.is_none()
        || (include_pack_body_cache
            && !engine_lock
                .as_ref()
                .is_some_and(IconSearchEngine::has_pack_body_cache));
    ...
    if should_load {
        ...
        *engine_lock = Some(if include_pack_body_cache {
            IconSearchEngine::load_fast_with_pack_body_cache(index_dir)?
        } else {
            IconSearchEngine::load_fast(index_dir)?
        });
    }
...
fn with_body_engine<F, R>(f: F) -> anyhow::Result<R>
where
    F: FnOnce(&IconSearchEngine) -> R,
{
    load_engine(true)?;
...
fn export_icons(
    ...
    let start = std::time::Instant::now();
    let mut results = with_body_engine(|engine| engine.search(query, limit * 10))?;
...
fn export_to_desktop(icon_specs: &[&str]) -> anyhow::Result<()> {
    ...
    for spec in icon_specs {
        ...
        let results = with_body_engine(|engine| engine.search(name, 100))?;
...
fn generate_svg(name: &str, pack: &str) -> anyhow::Result<String> {
    if let Some(svg) = generate_svg_from_engine_pack_body(name, pack)? {
        return Ok(svg);
    }

    generate_svg_from_json(name, pack)
}

fn generate_svg_from_engine_pack_body(name: &str, pack: &str) -> anyhow::Result<Option<String>> {
    let Some(resolved) = with_body_engine(|engine| engine.resolve_icon_body(pack, name))? else {
        return Ok(None);
    };
    ...
    Ok(Some(format_svg(width, height, &resolved.body)))
}

fn generate_svg_from_json(name: &str, pack: &str) -> anyhow::Result<String> {
    ...
    Ok(format_svg(width, height, body))
}

fn format_svg(width: impl std::fmt::Display, height: impl std::fmt::Display, body: &str) -> String {
    format!(
