...
impl BloomFilter {
    ...
    pub fn to_machine_parts(&self) -> (Vec<u64>, usize, usize) {
        (self.bits.clone(), self.num_hashes, self.size)
    }
    ...
    pub fn from_machine_parts(
        ...
        if size == 0 {
            return Err("filter size must be greater than zero".to_string());
        }
        if num_hashes == 0 {
            return Err("filter num_hashes must be greater than zero".to_string());
        }
        let expected_words = size.div_ceil(64);
        if bits.len() != expected_words {
            return Err(format!(
                "bloom filter bits length {} does not match expected words {}",
                bits.len(),
                expected_words
            ));
        }
...
impl IconBloomFilters {
    ...
    pub fn to_machine_parts(&self) -> Vec<(Vec<u64>, usize, usize)> {
        self.filters
            .iter()
            .map(BloomFilter::to_machine_parts)
            .collect()
    }
...
mod tests {
    ...
    #[test]
    fn test_bloom_filter_from_machine_parts_rejects_bad_shape() {
        let error = match BloomFilter::from_machine_parts(Vec::new(), 7, 128) {
            Ok(_) => panic!("bad bloom filter machine shape should be rejected"),
            Err(error) => error,
        };

        assert!(error.contains("bits length"));
    }
}
