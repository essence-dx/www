...
impl ExtensionApi for SecurityExtensionApi {
    ...
    fn get_decorations(&self, path: &Path) -> Vec<DecorationRange> {
        ...
        if let Ok(cache) = self.secrets_cache.read()
            && let Some(secrets) = cache.get(path)
        {
            for secret in secrets {
                decorations.push(DecorationRange {
                    start_line: secret.line_number.saturating_sub(1),
                    start_column: u32::from(secret.column),
                    end_line: secret.line_number.saturating_sub(1),
                    end_column: u32::from(secret.column) + 20, // Approximate secret length
        ...
        if let Ok(cache) = self.findings_cache.read()
            && let Some(findings) = cache.get(path)
        {
            for finding in findings {
                ...
                decorations.push(DecorationRange {
                    ...
                    start_column: u32::from(finding.column),
                    end_line: finding.line_number.saturating_sub(1),
                    end_column: u32::from(finding.column) + 10,
                    decoration_type,
                ...
                if secret.line_number == line + 1 {
                    ...
                    actions.push(CodeAction {
                        ...
                        edits: vec![TextEdit {
                            ...
                            start_column: u32::from(secret.column),
                            end_line: line,
                            end_column: u32::from(secret.column) + 40, // Approximate
                    ...
                    actions.push(CodeAction {
                        title: format!("Replace {} with environment variable", secret.secret_type),
                        kind: CodeActionKind::QuickFix,
                        file_path: path.to_path_buf(),
                        edits: vec![TextEdit {
                            start_line: line,
                            start_column: u32::from(secret.column),
                            end_line: line,
                            end_column: u32::from(secret.column) + 40,
...
mod tests {
    ...
    fn test_get_code_actions() {
        ...
        assert!(
            actions
                .iter()
                .any(|a| a.title.contains("environment variable"))
        );
    }
