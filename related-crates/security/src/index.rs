...
pub struct IndexStats {
    /// Number of vulnerability entries available in the index.
    pub entry_count: u32,
    /// Total serialized or mapped index size in bytes.
    pub size_bytes: usize,
    /// Hash seed used to compute package lookup slots.
    pub hash_seed: u64,
}
        ...
        if let Some(ref mmap) = self.mmap {
            ...
            if offset + VULN_ENTRY_SIZE <= mmap.len() {
                ...
                if let Some(vuln) = Vulnerability::from_bytes(entry_bytes)
                    && vuln.package_hash == package_hash
                {
...
fn map_read_only_file(path: &Path, file: &File) -> Result<Mmap> {
...
#[allow(unsafe_code)]
fn try_map_read_only_file(file: &File) -> std::io::Result<Mmap> {
    // SAFETY: the file handle is opened read-only and the returned mapping is exposed
    // only as immutable bytes, so this wrapper centralizes the mmap boundary.
