...
pub struct MapStats {
    ...
    /// Total bytes exposed through successful mappings.
    pub total_bytes: usize,
    /// Number of files that could not be mapped.
...
impl MemoryMapper {
    ...
    pub fn map_file(&mut self, path: &Path) -> Result<MappedFile> {
        ...
        let mmap = map_read_only_file(path, &file)?;
    ...
    fn map_file_static(path: &Path, stats: &AtomicMapStats) -> Option<MappedFile> {
        ...
        let mmap = match try_map_read_only_file(&file) {
            Ok(m) => m,
            Err(_) => {
                stats.failed_maps.fetch_add(1, Ordering::Relaxed);
                return None;
    ...
    pub fn map_directory_parallel(&mut self, path: &Path) -> Result<MappedDirectory> {
        ...
        let files: Vec<MappedFile> = file_paths
            .par_iter()
            .filter_map(|p| Self::map_file_static(p, &stats))
            .collect();
...
fn map_read_only_file(path: &Path, file: &std::fs::File) -> Result<Mmap> {
    try_map_read_only_file(file).map_err(|e| SecurityError::MapError {
        path: path.to_path_buf(),
        source: e,
    })
}
...
#[allow(unsafe_code)]
fn try_map_read_only_file(file: &std::fs::File) -> std::io::Result<Mmap> {
    // SAFETY: the file is mapped read-only and this crate exposes the mapping as
    // immutable bytes, keeping the unsafe mmap edge in one small helper.
