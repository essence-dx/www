...
pub const DXS_MAGIC: [u8; 4] = *b"SR\0\0";
...
impl SecurityReport {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let score = bytes[5];
        let timestamp_bytes: [u8; 8] = bytes[6..14].try_into().map_err(|_| {
            SecurityError::InvalidFormat("Invalid report timestamp bytes".to_string())
        })?;
        let timestamp = u64::from_le_bytes(timestamp_bytes);
        let mut git_hash = [0u8; 20];
        git_hash.copy_from_slice(&bytes[14..34]);
        let findings_count_bytes: [u8; 4] = bytes[34..38].try_into().map_err(|_| {
            SecurityError::InvalidFormat("Invalid findings count bytes".to_string())
        })?;
        let findings_count = u32::from_le_bytes(findings_count_bytes);
    ...
    pub fn import_verified(path: &Path, key: &VerifyingKey) -> Result<SecurityReport> {
        ...
        if let Some(signed_report) = signed {
            ...
            return Err(SecurityError::InvalidFormat(
                "Report is not signed".to_string(),
            ));
        }
...
impl MarkdownReportGenerator {
    ...
    fn build_document_with_paths(
        ...
        doc.nodes
            .push(DxmNode::Paragraph(vec![InlineNode::Bold(vec![
                InlineNode::Text(score_text),
            ])]));
        ...
        doc.nodes
            .push(DxmNode::Paragraph(vec![InlineNode::Text(format!(
                "Generated: {}",
                timestamp
            ))]));
