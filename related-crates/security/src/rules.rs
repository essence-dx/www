...
impl RuleEngine {
    ...
    pub fn load_rules(&mut self, path: &Path) -> Result<()> {
        ...
        if !header.is_valid() {
            return Err(SecurityError::RuleCompileError(
    ...
    fn find_next_rule_group(&self, current_pc: usize) -> usize {
        ...
        while pc < self.rules.len() {
            if let Some(op) = OpCode::from_u8(self.rules[pc].opcode)
                && matches!(op, OpCode::Accept | OpCode::End)
            {
                return pc + 1;
            }
            pc += 1;
    ...
    fn parse_yaml(yaml: &str) -> Result<Vec<YamlRule>> {
        ...
        for line in yaml.lines() {
            ...
            if line.starts_with("- id:") {
                ...
                // Start new rule
                let Some(id_source) = line.strip_prefix("- id:") else {
                    continue;
                };
                let id = id_source.trim().trim_matches('"');
