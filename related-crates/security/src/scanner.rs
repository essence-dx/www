...
pub enum SecretType {
    /// AWS access key identifier such as `AKIA...`.
    AwsAccessKey,
    /// AWS secret access key material.
    AwsSecretKey,
    /// GitHub personal, OAuth, or scoped token prefix.
    GitHubToken,
    /// Generic API key assignment or similarly named token.
    GenericApiKey,
    /// PEM or similar private key block.
    PrivateKey,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum SimdMode {
    /// AVX-512 accelerated scanner path.
    Avx512,
    /// AVX2 accelerated scanner path.
    Avx2,
    /// ARM NEON accelerated scanner path.
    Neon,
    /// Portable scalar fallback scanner path.
    Scalar,
}
        ...
        for i in 0..data.len() {
            ...
            for pattern in PATTERNS {
                    ...
                    && &data[i..i + pattern.prefix.len()] == pattern.prefix
                {
                    ...
                    if end - i >= pattern.min_length && (pattern.validator)(&data[i..end]) {
...
mod tests {
    ...
    fn test_detect_aws_access_key() {
        ...
        assert!(
            findings
                .iter()
                .any(|f| f.pattern_type == SecretType::AwsAccessKey),
    ...
    fn test_detect_github_token() {
        ...
        assert!(
            findings
                .iter()
                .any(|f| f.pattern_type == SecretType::GitHubToken),
    ...
    fn test_detect_private_key() {
        ...
        assert!(
            findings
                .iter()
                .any(|f| f.pattern_type == SecretType::PrivateKey),
    ...
    fn test_line_number_tracking() {
        ...
        if let Some(finding) = findings
            .iter()
            .find(|f| f.pattern_type == SecretType::AwsAccessKey)
        {
