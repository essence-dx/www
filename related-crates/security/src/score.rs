...
pub struct ScanFindings {
    ...
    /// Number of red supply-chain policy findings
    pub supply_chain_red_findings: u32,
    /// Number of yellow supply-chain policy findings
    pub supply_chain_yellow_findings: u32,
...
const SUPPLY_CHAIN_RED_PENALTY: i32 = 60;
const SUPPLY_CHAIN_YELLOW_PENALTY: i32 = 15;
const INOCULATION_BONUS: i32 = 5;
const XOR_VERIFIED_BONUS: i32 = 5;

/// Calculate security score from findings
///
/// Formula: clamp(100 - CVE/secret/supply-chain penalties + bonuses, 0, 100)
pub fn calculate_score(findings: &ScanFindings) -> u8 {
    ...
    score -= (findings.supply_chain_red_findings as i32) * SUPPLY_CHAIN_RED_PENALTY;
    score -= (findings.supply_chain_yellow_findings as i32) * SUPPLY_CHAIN_YELLOW_PENALTY;
...
mod tests {
    ...
    #[test]
    fn test_supply_chain_red_penalty_is_blocking() {
        let findings = ScanFindings {
            supply_chain_red_findings: 1,
            ..Default::default()
        };
    ...
    #[test]
    fn test_supply_chain_yellow_penalty_requires_review() {
        let findings = ScanFindings {
            supply_chain_yellow_findings: 1,
            ..Default::default()
        };
        assert_eq!(calculate_score(&findings), 85);
    }
...
mod property_tests {
    ...
    fn arb_scan_findings() -> impl Strategy<Value = ScanFindings> {
        (
            ...
            0u32..100,     // supply_chain_red_findings
            0u32..100,     // supply_chain_yellow_findings
            ...
            .prop_map(
                |(
                    ...
                    supply_chain_red_findings: supply_chain_red,
                    supply_chain_yellow_findings: supply_chain_yellow,
    ...
    proptest! {
        ...
        fn prop_score_matches_formula(findings in arb_scan_findings()) {
            ...
            expected -= (findings.supply_chain_red_findings as i32) * 60;
            expected -= (findings.supply_chain_yellow_findings as i32) * 15;
        ...
        #[test]
        fn prop_supply_chain_red_penalty_is_60(
            base in arb_scan_findings(),
            additional_red in 0u32..2
        ) {
            if base.supply_chain_red_findings < 2 && additional_red > 0 {
                let mut modified = base.clone();
                modified.supply_chain_red_findings += additional_red;

                let base_score = calculate_score(&base) as i32;
                let modified_score = calculate_score(&modified) as i32;
                let expected_diff = (additional_red as i32) * 60;
                let actual_diff = base_score - modified_score;

                if base_score < 100 && modified_score > 0 {
                    prop_assert_eq!(
                        actual_diff,
                        expected_diff,
                        "Supply-chain red penalty should be 60 per finding"
                    );
                }
            }
        }
        ...
        #[test]
        fn prop_supply_chain_yellow_penalty_is_15(
            base in arb_scan_findings(),
            additional_yellow in 0u32..4
        ) {
            if base.supply_chain_yellow_findings < 4 && additional_yellow > 0 {
                let mut modified = base.clone();
                modified.supply_chain_yellow_findings += additional_yellow;

                let base_score = calculate_score(&base) as i32;
                let modified_score = calculate_score(&modified) as i32;
                let expected_diff = (additional_yellow as i32) * 15;
                let actual_diff = base_score - modified_score;
                ...
                if base_score < 100 && modified_score > 0 {
                    prop_assert_eq!(
                        actual_diff,
                        expected_diff,
                        "Supply-chain yellow penalty should be 15 per finding"
                    );
                }
            }
        }
