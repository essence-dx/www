...
fn css_for(engine: &AnimationEngine, class_name: &str) -> Option<String> {
    let animation = engine.parse(class_name)?;
    let css = engine.generate_css(&animation);
...
fn main() {
    ...
    for anim in animations {
        if let Some(css) = css_for(&engine, anim) {
    ...
    if let Some(css) = css_for(&engine, full) {
        println!("   Class: {}", full);
    ...
    let presets = engine.preset_names();
