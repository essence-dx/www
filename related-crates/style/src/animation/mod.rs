...
impl AnimationEngine {
    ...
    pub fn generate_css(&self, anim: &AnimationClass) -> String {
        ...
        {
            (
                preset.default_duration.clone(),
                preset.default_easing.clone(),
            )
        } else {
...
mod tests {
    ...
    fn test_merge_keyframes() {
        ...
        let has_opacity = keyframes
            .iter()
            .any(|kf| kf.properties.iter().any(|(p, _)| p == "opacity"));
        let has_transform = keyframes
            .iter()
            .any(|kf| kf.properties.iter().any(|(p, _)| p == "transform"));
        assert!(has_opacity);
