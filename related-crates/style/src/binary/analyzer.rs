...
impl StyleAnalyzer {
    ...
    pub fn generate_combo_code(&self, top_n: usize, min_occurrences: usize) -> String {
        ...
        code.push_str("// Pattern → MacroID mapping\n");
    ...
    pub fn print_report(&self, top_n: usize, min_occurrences: usize) {
        ...
        println!(
            "Top {} patterns (≥{} occurrences):\n",
            top_n, min_occurrences
        );

        println!("┌─────┬────────┬─────────────────────────────────────────┬──────────┐");
        println!("│ ID  │ Uses   │ Pattern                                 │ Strategy │");
        println!("├─────┼────────┼─────────────────────────────────────────┼──────────┤");
        ...
        println!("  → Macro entries: {}", patterns.len());
        println!(
            "  → Individual IDs: {} → Macro IDs: {}",
            total_individual_ids, total_macro_ids
        );
        println!(
            "  → Bytes saved: {} bytes ({:.1}% reduction)",
            bytes_saved,
            (bytes_saved as f64 / (total_individual_ids * 2) as f64) * 100.0
        );
        println!("\n🎯 STRATEGY:");
        println!(
            "  • Frequent patterns (≥{} uses) → Auto-grouped as MACROs",
            min_occurrences
        );
        println!("  • Rare patterns → Kept ATOMIC for flexibility");
        println!("  • CSS file size: ~5 KB (gzipped)");
...
mod tests {
    ...
    fn test_scan_html() {
        ...
        let flex_pattern = patterns
            .iter()
            .find(|p| p.classes == vec!["flex", "items-center", "p-4"]);
