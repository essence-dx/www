...
impl<'a> BinaryDawnReader<'a> {
    ...
    pub fn new(data: &'a [u8]) -> Result<Self, BinaryDawnError> {
        ...
        if data.len() < BinaryDawnHeader::SIZE {
            error!(
                data_len = data.len(),
                "Binary Dawn data too short for header"
            );
            return Err(BinaryDawnError::HeaderTooShort);
        }
...
mod tests {
    ...
    fn test_roundtrip() {
        ...
        assert_eq!(
            reader.get_css(42),
            Some(".container { max-width: 1200px; }")
        );
        assert_eq!(
            reader.get_css(127),
            Some(".text-center { text-align: center; }")
        );
        assert_eq!(reader.get_css(128), Some(".bg-blue { background: blue; }"));
        assert_eq!(
            reader.get_css(1000),
            Some(".complex { color: red; font-size: 16px; }")
        );
    }
