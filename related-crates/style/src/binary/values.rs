...
fn get_value_string(prop: CssProperty, val: u8) -> String {
    match prop {
        CssProperty::Display => DISPLAY_VALUES
            .get(val as usize)
            .unwrap_or(&"block")
            .to_string(),
        CssProperty::FlexDirection => FLEX_DIRECTION_VALUES
            .get(val as usize)
            .unwrap_or(&"row")
            .to_string(),
        CssProperty::JustifyContent => JUSTIFY_CONTENT_VALUES
            .get(val as usize)
            .unwrap_or(&"flex-start")
            .to_string(),
        CssProperty::AlignItems => ALIGN_ITEMS_VALUES
            .get(val as usize)
            .unwrap_or(&"stretch")
            .to_string(),
        CssProperty::Position => POSITION_VALUES
            .get(val as usize)
            .unwrap_or(&"static")
            .to_string(),
...
mod tests {
    ...
    fn test_roundtrip() {
        let original_props = vec![
            ...
            (
                ...
                JustifyContentValue::Center as u8,
            ),
            (CssProperty::AlignItems, AlignItemsValue::Center as u8),
        ];
    ...
    fn test_invalid_property_bytes_return_errors() {
        ...
        assert!(
            CssProperty::try_from(0x00).is_err(),
            "Byte 0x00 should return error"
        );
        ...
        for byte in 0x26u8..=0xFF {
            ...
            assert!(
                result.is_err(),
                "Byte 0x{:02X} should return error, got {:?}",
                byte,
                result
            );
