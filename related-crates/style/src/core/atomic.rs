...
mod tests {
    ...
    fn test_lookup_performance() {
        ...
        // Warm up cache
        for _ in 0..100 {
            let _ = lookup_atomic_class(black_box("flex"));
        }
        ...
        for _ in 0..iterations {
            black_box(lookup_atomic_class(black_box("flex")));
        }
        let elapsed = start.elapsed();
        let per_lookup = elapsed.as_nanos() / iterations;

        // Should be well under 1µs (1000ns)
        println!("Average lookup time: {}ns", per_lookup);
        // This is a unit-test smoke check; tighter budgets belong in real benchmarks.
        assert!(
            per_lookup < 2_500,
            "Lookup took {}ns, expected <2500ns",
            per_lookup
        );
    }
}
