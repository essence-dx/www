...
pub fn generate_color_css(engine: &StyleEngine, class_name: &str) -> Option<String> {
    ...
    for (prefix, property) in color_rules {
        let Some(raw_name) = class_name.strip_prefix(prefix) else {
            continue;
        };
        let (name, alpha) = split_theme_color_alpha(raw_name)?;
        if alpha.is_some() {
            if let Some(css) =
                crate::core::engine::utility::tailwind_v43_palette_opacity_property_css(
                    property, raw_name,
                )
            {
                return Some(css);
        ...
        if derive_color_value(engine, name).is_some() {
            return Some(format!(
                "{}: {}",
                property,
                theme_color_reference(name, alpha.as_deref())
            ));
        }
    }
    None
...
fn split_theme_color_alpha(raw_name: &str) -> Option<(&str, Option<String>)> {
    let (name, alpha) = match raw_name.rsplit_once('/') {
        Some((name, alpha)) if !name.is_empty() => {
            if is_opaque_color_alpha_token(alpha) {
                (name, None)
            } else {
                (name, Some(color_alpha_percentage(alpha)?))
            }
        }
        Some(_) => return None,
        None => (raw_name, None),
...
fn theme_color_reference(name: &str, alpha: Option<&str>) -> String {
    let color = format!("var(--color-{name})");
    if let Some(alpha) = alpha {
        format!("color-mix(in oklab, {color} {alpha}, transparent)")
    } else {
        color
    }
}

fn color_alpha_percentage(raw_alpha: &str) -> Option<String> {
    let alpha = raw_alpha
        .strip_prefix('[')
    ...
    if let Some(percent) = alpha.strip_suffix('%') {
        ...
        if (0.0..=100.0).contains(&value) {
            return Some(format!("{}%", trim_color_alpha(value)));
        }
        return None;
    }
    ...
    Some(format!("{}%", trim_color_alpha(percent)))
}

fn trim_color_alpha(value: f32) -> String {
...
fn is_opaque_color_alpha_token(raw_alpha: &str) -> bool {
    raw_alpha
        .trim()
        .parse::<f32>()
