...
impl CssPropertyGenerator {
    ...
    fn properties_to_document(&self, properties: &[CssPropertyDef]) -> DxDocument {
        ...
        for prop in properties {
            ...
            doc.context.insert(
                format!("p:{}|cat", prop.name),
                DxLlmValue::Str(prop.category.clone()),
            );
            ...
            doc.context.insert(
                format!("p:{}|num", prop.name),
                DxLlmValue::Bool(prop.accepts_numeric),
            );
            ...
            doc.context
                .insert(format!("p:{}|bs", prop.name), DxLlmValue::Str(bs_str));
        }
