...
impl GroupRegistry {
    ...
    pub fn analyze(
        ...
        if let Some(engine) = engine {
            ...
            known_prefixes.extend(engine.custom_variants.keys().cloned());
    ...
    pub fn generate_css_for<'a>(
        ...
        if !simple_bodies.is_empty() {
            let selector_output = combined_selector
                .as_deref()
                .unwrap_or(alias_selector.as_str());
...
mod tests {
    ...
    fn test_stable_classname_tracking() {
        ...
        assert_eq!(
            registry.get_stable_classname(&utilities),
    ...
    fn test_update_group_css() {
        ...
        registry
            .cached_css
            .insert("card".to_string(), ".card { ... }".to_string());
