...
mod tests {
    ...
    static TEST_CACHE_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

    #[test]
    fn test_memoization() {
        let _guard = TEST_CACHE_LOCK.lock().expect("cache test lock poisoned");
        clear_cache();
    ...
    fn test_cache_stats() {
        ...
        assert_eq!(
            get_or_generate_css("class1", || Some("unexpected".to_string())),
            Some("css1".to_string())
        );
        assert_eq!(
            get_or_generate_css("class2", || Some("unexpected".to_string())),
            Some("css2".to_string())
        );
