...
pub fn generate_css(
    ...
    let classes = classes_with_source_inline(classes, engine);
...
fn classes_with_source_inline<'a>(
    classes: &'a AHashSet<String>,
    engine: &'a StyleEngine,
) -> Vec<&'a String> {
    let mut combined =
        Vec::with_capacity(classes.len() + engine.source_inline_class_tokens().len());

    for class in classes {
        if engine.class_is_source_inline_excluded(class) {
            continue;
        }
        combined.push(class);
    }

    for class in engine.source_inline_class_tokens() {
        if engine.class_is_source_inline_excluded(class) || classes.contains(class) {
            continue;
        }
        combined.push(class);
    }
