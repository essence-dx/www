//! DX style engine for generated CSS, utility parsing, and serializer-backed receipts.
//!
...
pub fn compile(input: PathBuf, output: PathBuf) -> Result<(), StyleError> {
    ...
    debug!(
        class_count = extracted.classes.len(),
        "Extracted classes from HTML"
    );
    ...
    debug!(
        unique_classes = sorted_classes.len(),
        "Processing sorted classes"
    );
