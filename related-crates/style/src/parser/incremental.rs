...
use super::{ExtractedClasses, GroupEvent, SourceScanDiagnostic, extract_classes_fast};
...
struct RegionCache {
    ...
    /// Source scanner diagnostics in this region
    source_diagnostics: Vec<SourceScanDiagnostic>,
}
...
impl IncrementalParser {
    ...
    fn parse_changed_regions(
        ...
        let mut all_source_diagnostics = Vec::new();
        ...
        for &(start, end) in changed_regions {
            ...
            all_source_diagnostics.extend(result.source_diagnostics);
        ...
        for cached_region in &self.regions {
            ...
            if !overlaps {
                ...
                all_source_diagnostics.extend(cached_region.source_diagnostics.iter().cloned());
        ...
        ExtractedClasses {
            ...
            source_diagnostics: all_source_diagnostics,
        }
    }
    ...
    fn update_cache(&mut self, content: &[u8], hash: u64, result: &ExtractedClasses) {
        ...
        let region = RegionCache {
            ...
            source_diagnostics: result.source_diagnostics.clone(),
        };
    ...
    fn reconstruct_from_cache(&mut self) -> ExtractedClasses {
        ...
        for region in &self.regions {
            ...
            source_diagnostics.extend(region.source_diagnostics.iter().cloned());
        }
