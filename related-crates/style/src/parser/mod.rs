...
pub struct ExtractedClasses {
    ...
    pub source_diagnostics: Vec<SourceScanDiagnostic>,
}

#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub enum SourceScanDiagnosticKind {
    DynamicFragment,
    ObjectKey,
    UnsafeCandidate,
    NonUtilityCandidate,
...
impl SourceScanDiagnosticKind {
    pub fn as_str(self) -> &'static str {
        match self {
            Self::DynamicFragment => "dynamic-fragment",
            Self::ObjectKey => "object-key",
            Self::UnsafeCandidate => "unsafe-candidate",
            Self::NonUtilityCandidate => "non-utility-candidate",
...
pub struct SourceScanDiagnostic {
    pub kind: SourceScanDiagnosticKind,
    pub token: String,
    pub line: usize,
    pub column: usize,
    pub message: String,
...
#[inline]
fn strip_comment_outside_brackets(s: &str) -> &str {
    ...
    for (index, byte) in s.bytes().enumerate() {
        match byte {
            ...
            b'#' if bracket_depth == 0 => return &s[..index],
            _ => {}
        }
    }
...
fn expand_grouping_into(s: &str, out: &mut AHashSet<String>, collector: &mut GroupCollector) {
    let s = strip_comment_outside_brackets(s);
    if !s
        .as_bytes()
        .iter()
...
pub fn extract_classes_fast(html_bytes: &[u8], capacity_hint: usize) -> ExtractedClasses {
    ...
    while let Some(idx) = class_finder.find(&html_bytes[pos..]) {
        ...
        let mut i = attr_start + 5;
        if i + 4 <= n && html_bytes[i..i + 4].eq_ignore_ascii_case(b"Name") {
            i += 4;
        }
    ...
    if let Ok(source) = std::str::from_utf8(html_bytes) {
        let report = plain_text::scan_plain_text_class_tokens(source);
        for token in report.tokens {
            set.insert(token);
        }
        source_diagnostics = report.diagnostics;
    }
...
mod tests {
    ...
    #[test]
    fn extracts_jsx_class_name_attributes() {
        let html = br#"<Button className="flex p-4 hover:bg-blue-500 md:grid-cols-2" data-class="ignored"></Button>"#;
        let extracted = extract_classes_fast(html, 0);

        assert!(extracted.classes.contains("flex"));
        assert!(extracted.classes.contains("p-4"));
        assert!(extracted.classes.contains("hover:bg-blue-500"));
        assert!(extracted.classes.contains("md:grid-cols-2"));
        assert!(
            !extracted.classes.contains("ignored"),
            "attribute names containing class should not be treated as class attributes"
        );
    }

    #[test]
    fn extracts_arbitrary_hex_classes_without_comment_truncation() {
        let html = br##"<main className="bg-[#030303] text-[#fafafa] border-zinc-800"></main>"##;
        let extracted = extract_classes_fast(html, 0);

        assert!(extracted.classes.contains("bg-[#030303]"));
        assert!(extracted.classes.contains("text-[#fafafa]"));
        assert!(extracted.classes.contains("border-zinc-800"));
    }

    #[test]
    fn extracts_plain_text_static_class_literals_from_tsx_sources() {
        let source = br#"
const variants = {
  "grid": "grid-cols-2",
...
const states = ["data-[state=open]:block", "px-4"];
const dynamic = `text-${tone}-600`;
"#;
        let extracted = extract_classes_fast(source, 0);
        ...
        for expected in [
            ...
            assert!(
                extracted.classes.contains(expected),
                "expected static literal token {expected}: {:?}",
                extracted.classes
            );
        }
        assert!(
            !extracted.classes.contains("grid"),
            "object-map keys should not be treated as class tokens"
        );
        assert!(!extracted.classes.iter().any(|token| token.contains("${")));
    }

    #[test]
    fn reports_plain_text_source_scan_diagnostics_with_locations() {
        let source = br#"
const variants = { "grid": "grid-cols-2" };
const dynamic = `text-${tone}-600`;
const unsafeUrl = "https://example.com";
const prose = "card shell copy";
const duplicate = ["px-4", "px-4"];
"#;
        let extracted = extract_classes_fast(source, 0);

        assert!(extracted.classes.contains("grid-cols-2"));
        assert!(extracted.classes.contains("px-4"));
            ...
            assert!(
                extracted
                    ...
                    .any(|diagnostic| diagnostic.kind == expected
                        && diagnostic.line > 0
                        && diagnostic.column > 0),
                "expected located diagnostic {expected:?}: {:?}",
                extracted.source_diagnostics
            );
        }
    }
