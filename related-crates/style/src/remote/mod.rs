...
impl RemoteImporter {
    ...
    pub fn parse_style_definition(content: &str) -> Result<StyleDefinition, ImportError> {
        ...
        if css.is_empty() && classes.is_empty() {
            return Err(ImportError::InvalidFormat(
                "Missing 'css' or 'classes' field".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_cache_key_dx_registry() {
        ...
        assert_eq!(
            RemoteImporter::cache_key(&ref_versioned),
            "dx_alice_button_v1.0.0"
        );
    }
    ...
    fn test_cache_key_github() {
        ...
        assert_eq!(
            RemoteImporter::cache_key(&ref_),
            "gh_owner_repo_styles_button.sr_main"
        );
    }
