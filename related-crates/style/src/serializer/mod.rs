...
impl StyleConfig {
    ...
    pub fn to_dx_document(&self) -> DxDocument {
        ...
        for (name, css) in &self.static_styles {
            doc.context
                .insert(format!("s:{}", name), DxLlmValue::Str(css.clone()));
        }
        ...
        for (name, value) in &self.screens {
            doc.context
                .insert(format!("sc:{}", name), DxLlmValue::Str(value.clone()));
        }

        // States: st:name -> value
        for (name, value) in &self.states {
            doc.context
                .insert(format!("st:{}", name), DxLlmValue::Str(value.clone()));
        }

        // Container queries: cq:name -> value
        for (name, value) in &self.container_queries {
            doc.context
                .insert(format!("cq:{}", name), DxLlmValue::Str(value.clone()));
        }

        // Colors: c:name -> value
        for (name, value) in &self.colors {
            doc.context
                .insert(format!("c:{}", name), DxLlmValue::Str(value.clone()));
        }

        // Animation generators: ag:name -> template
        for (name, template) in &self.animation_generators {
            doc.context
                .insert(format!("ag:{}", name), DxLlmValue::Str(template.clone()));
        }
        ...
        for (alias, def) in &self.groups.definitions {
            ...
            doc.context.insert(
                format!("gd:{}|u", alias),
                DxLlmValue::Str(def.utilities.join("|")),
            );
            doc.context.insert(
                format!("gd:{}|e", alias),
                DxLlmValue::Bool(def.allow_extend),
            );
            doc.context.insert(
                format!("gd:{}|r", alias),
                DxLlmValue::Str(def.raw_tokens.join("|")),
            );
            doc.context.insert(
                format!("gd:{}|d", alias),
                DxLlmValue::Str(def.dev_tokens.join("|")),
            );
        }
        ...
        for (alias, css) in &self.groups.cached_css {
            doc.context
                .insert(format!("gc:{}", alias), DxLlmValue::Str(css.clone()));
        }
        ...
        if !self.base_css.is_empty() {
            doc.context.insert(
                "base_css".to_string(),
                DxLlmValue::Str(self.base_css.clone()),
            );
        }
        if !self.property_css.is_empty() {
            doc.context.insert(
                "property_css".to_string(),
                DxLlmValue::Str(self.property_css.clone()),
            );
        }
    ...
    pub fn from_dx_document(doc: &DxDocument) -> Result<Self, SerializerError> {
            ...
            if let Some(name) = key.strip_prefix("s:") {
                ...
                if let DxLlmValue::Str(css) = value {
                    config
                        .groups
                        .cached_css
                        .insert(alias.to_string(), css.clone());
                }
        ...
        for ((key_name, property), values) in dynamic_values {
            config
                .dynamic_styles
                .insert(key_name, DynamicEntry { property, values });
        }
...
mod tests {
    ...
    fn test_static_styles_roundtrip() {
        ...
        config
            .static_styles
            .insert("flex".to_string(), "display: flex;".to_string());
        config
            .static_styles
            .insert("hidden".to_string(), "display: none;".to_string());
    ...
    fn test_groups_roundtrip() {
        ...
        let card_def = restored
            .groups
            .definitions
            .get("card")
            .expect("card definition should exist");
        assert_eq!(
            card_def.utilities,
            vec!["bg-white".to_string(), "rounded".to_string()]
        );
        assert_eq!(
            card_def.raw_tokens,
            vec!["card(bg-white rounded)".to_string()]
        );
        assert_eq!(
            card_def.dev_tokens,
            vec!["bg-white".to_string(), "rounded".to_string()]
...
mod prop_tests {
    ...
    fn arb_group_def_dump() -> impl Strategy<Value = GroupDefDump> {
        (
            ...
            .prop_map(
                |(utilities, allow_extend, raw_tokens, dev_tokens)| GroupDefDump {
                    utilities,
                    allow_extend,
                    raw_tokens,
                    dev_tokens,
