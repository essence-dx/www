...
fn test_invalid_binary_format_parsing() {
    ...
    assert!(
        result.is_err(),
        "Should return error for invalid binary format"
    );
...
fn test_style_error_display() {
    ...
    assert!(
        ...
        "Error display should contain file path"
    );
    ...
    assert!(
        display.contains("10"),
        "Error display should contain line number"
    );
    assert!(
        display.contains("5"),
        "Error display should contain column number"
    );
...
fn test_html_parse_error_display() {
    ...
    assert!(
        display.contains("42"),
        "Error display should contain the line number"
    );
}
