...
fn test_html_to_css_compilation() {
    ...
    assert!(
        !extracted.classes.is_empty(),
        "Should extract classes from HTML"
    );

    // Verify specific classes are present
    let classes: HashSet<_> = extracted.classes.iter().collect();
    assert!(
        classes.contains(&"flex".to_string()),
        "Should contain 'flex' class"
    );
    assert!(
    ...
    assert!(
        classes.contains(&"p-4".to_string()),
        "Should contain 'p-4' class"
    );
    assert!(
...
fn test_binary_dawn_round_trip() {
    ...
    assert!(
        ...
        "Binary Dawn data should not be empty"
    );
    ...
    for (id, expected_css) in &test_entries {
        let actual_css = reader
            .get_css(*id)
            .expect(&format!("Should find CSS for ID {}", id));
...
fn test_empty_html_handling() {
    ...
    assert!(
        extracted.classes.is_empty(),
        "Empty HTML should produce no classes"
    );
    ...
    assert!(
        extracted.classes.is_empty(),
        "HTML without classes should produce no classes"
    );
    ...
    assert!(
        extracted.classes.is_empty(),
        "Empty class attribute should produce no classes"
    );
}
...
fn test_large_file_handling() {
    ...
    assert!(
        !extracted.classes.is_empty(),
        "Should extract classes from large HTML"
    );

    // Verify some expected classes are present
    let classes: HashSet<_> = extracted.classes.iter().collect();
    assert!(
        classes.contains(&"flex-0".to_string()),
        "Should contain 'flex-0'"
    );
...
fn test_auto_grouping_html_output() {
    ...
    if let Some(rewrite) = result {
        ...
        assert!(
            rewritten.contains("class="),
            "Rewritten HTML should contain class attributes"
        );

        // Verify grouped classnames start with dxg-
        for group in &rewrite.groups {
            assert!(
                group.alias.starts_with("dxg-"),
                "Grouped classname should start with 'dxg-'"
            );
        }
    }
}
...
fn test_auto_grouping_preserves_non_grouped() {
    ...
    assert!(
        result.is_none(),
        "No groups should be created with high threshold"
    );
}
