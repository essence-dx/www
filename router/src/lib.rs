...
//!
//! File-based routing utilities for DX-WWW compatibility and route tests.
//!
//! ## File Mapping
//!
//! | File Path | Route |
...
//! | `pages/docs/[...path].html` | `/docs/*` |
//! | `pages/(marketing)/pricing.html` | `/pricing` |
//!
//! ## Usage
//!
//! ```rust,ignore
...
impl Route {
    ...
    pub fn new(pattern: &str, file_path: PathBuf) -> RouterResult<Self> {
        ...
        let is_catch_all = segments
            .iter()
            .any(|s| matches!(s, Segment::CatchAll(_) | Segment::OptionalCatchAll(_)));
    ...
    for entry in WalkDir::new(pages_dir).into_iter().filter_map(|e| e.ok()) {
        ...
        if let Some(name) = path.file_name().and_then(|n| n.to_str()) {
            if name.starts_with('_') {
                continue; // _layout.html, _error.html, _loading.html
            }
        }
...
mod tests {
    ...
    fn test_static_route() {
        let route = Route::new("/about", PathBuf::from("pages/about.html")).unwrap();
    ...
    fn test_dynamic_route() {
        let route = Route::new("/blog/[slug]", PathBuf::from("pages/blog/[slug].html")).unwrap();
    ...
    fn test_catch_all_route() {
        let route = Route::new(
            "/docs/[...path]",
            PathBuf::from("pages/docs/[...path].html"),
        )
        .unwrap();
    ...
    fn test_router_match_static() {
        let router = Router::from_routes(vec![
            Route::new("/", PathBuf::from("pages/index.html")).unwrap(),
            Route::new("/about", PathBuf::from("pages/about.html")).unwrap(),
        ]);
    ...
    fn test_router_match_dynamic() {
        let router = Router::from_routes(vec![
            Route::new("/blog/[slug]", PathBuf::from("pages/blog/[slug].html")).unwrap(),
        ]);
    ...
    fn test_specificity_ordering() {
        let router = Router::from_routes(vec![
            Route::new("/blog/[slug]", PathBuf::from("pages/blog/[slug].html")).unwrap(),
            Route::new("/blog/featured", PathBuf::from("pages/blog/featured.html")).unwrap(),
        ]);
    ...
    fn test_file_path_to_pattern() {
        ...
        assert_eq!(
            file_path_to_pattern(pages_dir, Path::new("www/pages/index.html")).unwrap(),
            "/"
        );
        assert_eq!(
            file_path_to_pattern(pages_dir, Path::new("www/pages/about.html")).unwrap(),
            "/about"
        );
        assert_eq!(
            file_path_to_pattern(pages_dir, Path::new("www/pages/blog/index.html")).unwrap(),
            "/blog"
        );
        assert_eq!(
            file_path_to_pattern(pages_dir, Path::new("www/pages/blog/[slug].html")).unwrap(),
            "/blog/[slug]"
        );
    }
}
