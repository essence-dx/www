...
impl ServerState {
    ...
    pub fn load_artifacts(&self, path: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if layout_path.exists() {
            ...
            self.binary_cache
                .insert("layout.bin".to_string(), bytes.clone());
            ...
            self.current_version
                .insert("layout.bin".to_string(), hash.clone());
        ...
        if wasm_path.exists() {
            ...
            let size = bytes.len();
            self.binary_cache
                .insert("app.wasm".to_string(), bytes.clone());
            ...
            self.current_version
                .insert("app.wasm".to_string(), hash.clone());
...
pub fn build_router_with_health_checker(
    ...
    router = router.route(
        "/health/ready",
        get(ops::readiness_handler).with_state(health_checker),
    );
