...
fn create_test_server_with_artifacts() -> TestServer {
    ...
    state
        .binary_cache
        .insert("app.wasm".to_string(), vec![0x00, 0x61, 0x73, 0x6d]); // WASM magic

    // Add version hashes
    state
        .current_version
        .insert("layout.bin".to_string(), "abc123".to_string());
    state
        .current_version
        .insert("app.wasm".to_string(), "def456".to_string());
...
mod ssr_bot_detection {
    ...
    async fn bot_receives_ssr_html() {
        ...
        assert!(
            ...
            "Bot should receive HTML content"
        );
    ...
    async fn bingbot_receives_ssr_html() {
        ...
        assert!(
            content_type.contains("text/html"),
            "Bingbot should receive HTML content"
        );
    }
    ...
    async fn slackbot_receives_ssr_html() {
        ...
        if status == axum::http::StatusCode::OK {
            ...
            assert!(
                content_type.contains("text/html"),
                "Slackbot should receive HTML content"
            );
        }
    }
    ...
    async fn twitterbot_receives_ssr_html() {
        ...
        assert!(
            content_type.contains("text/html"),
            "Twitterbot should receive HTML content"
        );
    }
    ...
    async fn human_receives_spa_shell() {
        ...
        assert!(
            content_type.contains("text/html"),
            "Human should receive HTML content"
        );
        ...
        assert!(
            ...
            "Should receive the SPA shell content"
        );
    ...
    async fn firefox_receives_spa_shell() {
        ...
        assert!(
            body.contains("SPA Shell"),
            "Firefox should receive SPA shell"
        );
    ...
    async fn safari_receives_spa_shell() {
        ...
        assert!(
            body.contains("SPA Shell"),
            "Safari should receive SPA shell"
        );
...
mod authentication {
    ...
    async fn token_refresh_returns_new_access_token() {
        ...
        if status == axum::http::StatusCode::OK {
            ...
            assert_eq!(
                refresh_body.get("token_type").and_then(|v| v.as_str()),
                Some("Bearer")
            );
        }
    }
...
mod security_headers {
    ...
    async fn x_frame_options_header_present() {
        ...
        if let Some(xfo) = response.headers().get("x-frame-options") {
            let xfo_value = xfo
                .to_str()
                .expect("X-Frame-Options should be valid string");
            assert!(
    ...
    async fn x_content_type_options_header_present() {
        ...
        if let Some(xcto) = response.headers().get("x-content-type-options") {
            let xcto_value = xcto
                .to_str()
                .expect("X-Content-Type-Options should be valid string");
            assert_eq!(
                xcto_value, "nosniff",
