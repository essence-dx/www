...
Flow can run small UI-specialized GGUF models locally through Rust/llama.tsxp. The default UI model is `Qwendean-4B` `Q4_K_M`, stored at `models/llm/Qwendean-4B.Q4_K_M.gguf`; `WEBGEN-4B-Preview` i1 is also installed as the website-generation candidate at `models/llm/WEBGEN-4B-Preview.i1-Q4_K_M.gguf`.
...
`FLOW_LLAMA_GPU_LAYERS` can be set to opt into llama.tsxp GPU offload. Leave it unset on low-VRAM Windows machines.
